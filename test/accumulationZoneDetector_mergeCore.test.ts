...
describe("AccumulationZoneDetector - Core Merge Functionality", () => {
    ...
    describe("Zone Merge Logic Validation", () => {
        it("should properly update existing zone when merge occurs", () => {
            ...
            const firstZoneTrades = createConcentratedTrades(basePrice, baseTime, 12, 0.8);
            firstZoneTrades.forEach(trade => detector.analyze(trade));
            
            // Trigger first zone formation
            const zone1Trigger = createTrade(basePrice, baseTime + 125000, true, 100);
            ...
            const overlappingTrades = createConcentratedTrades(overlappingPrice, baseTime + 130000, 10, 0.75);
            ...
            const mergeTrigger = createTrade(overlappingPrice, baseTime + 255000, true, triggerVolume);
        ...
        it("should create new zone when no overlapping zones exist", () => {
            ...
            const trades1 = createConcentratedTrades(basePrice, baseTime, 12, 0.8);
            ...
            const trades2 = createConcentratedTrades(distantPrice, baseTime + 130000, 12, 0.8);
            trades2.forEach(trade => detector.analyze(trade));
            
            const zone2Trigger = createTrade(distantPrice, baseTime + 255000, true, 100);
        ...
        it("should handle merge errors gracefully", () => {
            ...
            const overlappingTrades = createConcentratedTrades(basePrice + 200, baseTime + 130000, 10, 0.8);
            overlappingTrades.forEach(trade => detector.analyze(trade));
            
            // Trigger merge (should fail gracefully)
            const mergeTrigger = createTrade(basePrice + 200, baseTime + 255000, true, 80);
        ...
        it("should validate zone state consistency after merge", () => {
            ...
            const firstTrades = createConcentratedTrades(basePrice, baseTime, 15, 0.85); // High sell ratio
            firstTrades.forEach(trade => detector.analyze(trade));
            
            const trigger1 = createTrade(basePrice, baseTime + 125000, true, 120);
            ...
            const overlappingTrades = createConcentratedTrades(basePrice + 150, baseTime + 130000, 8, 0.75);
            const overlappingVolume = overlappingTrades.reduce((sum, t) => sum + t.quantity, 0);
            
            overlappingTrades.forEach(trade => detector.analyze(trade));
            
            // Trigger merge
            const triggerVolume = 90;
            const mergeTrigger = createTrade(basePrice + 150, baseTime + 255000, true, triggerVolume);
            ...
            expect(finalZone.strength).toBeGreaterThanOrEqual(initialStrength * 0.9); // Allow slight degradation
