...
const createZoneDebugLogger = (): ILogger => ({
    info: vi.fn((msg, data) => {
        console.log(`[ZONE-DEBUG-INFO] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    warn: vi.fn((msg, data) => {
        console.log(`[ZONE-DEBUG-WARN] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    error: vi.fn((msg, data) => {
        console.log(`[ZONE-DEBUG-ERROR] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    debug: vi.fn((msg, data) => {
        console.log(`[ZONE-DEBUG-DEBUG] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
});
...
describe("AbsorptionDetector - Zone Debug", () => {
    ...
    it("should verify zone aggregation and checkForSignal execution", { timeout: 5000 }, () => {
        ...
        const checkForSignalSpy = vi.spyOn(detector as any, 'checkForSignal');
        const analyzeZoneForAbsorptionSpy = vi.spyOn(detector as any, 'analyzeZoneForAbsorption');
        ...
        debugTrades.forEach((trade, i) => {
            ...
            console.log(`  Trade ${i + 1}: SELL ${trade.quantity} at ${basePrice}`);
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                console.log(`\n🔍 ZONE DEBUG RESULTS:`);
                console.log(`  checkForSignal called: ${checkForSignalSpy.mock.calls.length} times`);
                console.log(`  analyzeZoneForAbsorption called: ${analyzeZoneForAbsorptionSpy.mock.calls.length} times`);
                ...
                if (zoneAgg.size > 0) {
                    ...
                    for (const [zone, bucket] of zoneAgg) {
                        console.log(`    Zone ${zone}: ${bucket.trades.length} trades, vol ${bucket.vol}`);
                    }
                } else {
                    console.log(`  ❌ CRITICAL: No zones in zoneAgg - trades not being added!`);
                }
                
                if (checkForSignalSpy.mock.calls.length === 0) {
                    console.log(`\n🚨 ISSUE: checkForSignal never called`);
                    console.log(`  This suggests the BaseDetector.addTrade → checkForSignal chain is broken`);
                } else {
                    console.log(`\n✅ checkForSignal is working`);
                    if (analyzeZoneForAbsorptionSpy.mock.calls.length === 0) {
                        console.log(`  But analyzeZoneForAbsorption never called`);
                        console.log(`  This suggests empty zones or early returns in checkForSignal`);
                    } else {
                        console.log(`  analyzeZoneForAbsorption is also working`);
                        console.log(`  Issue must be in signal generation logic`);
                    }
                }
