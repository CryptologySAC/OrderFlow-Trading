...
describe("AbsorptionDetector - Validation Check", () => {
    ...
    it(
        "should check if FinancialMath validation is blocking execution",
        { timeout: 10000 },
        async () => {
            console.log(
            ...
            if (!priceValid || !quantityValid) {
                console.log(
                    `🚨 VALIDATION FAILURE: FinancialMath validation blocking!`
                );
                return;
            }

            console.log(
                `✅ FinancialMath validation passes - issue is elsewhere`
            );
            ...
            const testEvent = createValidationTestEvent(
                testPrice,
                testQuantity,
                baseTime,
                true
            ...
            return new Promise<void>((resolve) => {
                setTimeout(() => {
                    ...
                    if (errorCalls.length > 0) {
                        ...
                        console.log(
                            `✅ No validation errors - execution should proceed`
                        );
                        console.log(
                            `  This suggests the blocking issue is in the parent class flow`
                        );
                    }
