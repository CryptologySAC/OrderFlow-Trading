...
describe("AbsorptionDetector - REALISTIC Scenarios", () => {
    ...
    it(
        "should detect BUY signal when institutions absorb panic selling at market bottom",
        { timeout: 5000 },
        async () => {
            console.log(`\n🎯 ===== REALISTIC MARKET BOTTOM SCENARIO =====`);
            console.log(
                `📊 Context: Panic selling being absorbed by institutional buyers`
            );
            ...
            for (const [i, trade] of trades.entries()) {
                ...
                console.log(
                    `    Bid: ${trade.bidVol} (${trade.bidVol > (trades[i - 1]?.bidVol || 5000) ? "↑" : "↓"}) - ${trade.desc}`
                );
            ...
            console.log(
                `  Bid liquidity: 5000 → 5300 (increased despite selling!)`
            );
    ...
    it(
        "should detect SELL signal when institutions distribute during retail FOMO at market top",
        { timeout: 5000 },
        async () => {
            console.log(`\n🎯 ===== REALISTIC MARKET TOP SCENARIO =====`);
            console.log(
                `📊 Context: FOMO buying being absorbed by institutional sellers`
            );
            ...
            console.log(
                `   - Ask liquidity gets hit but REFILLS (distribution)`
            );
            ...
            for (const [i, trade] of trades.entries()) {
                ...
                console.log(
                    `    Ask: ${trade.askVol} (${trade.askVol > (trades[i - 1]?.askVol || 5000) ? "↑" : "↓"}) - ${trade.desc}`
                );
            ...
            console.log(
                `  Ask liquidity: 5000 → 5300 (increased despite buying!)`
            );
    ...
    it(
        "should NOT generate signals when no absorption pattern exists",
        { timeout: 5000 },
        async () => {
            console.log(`\n🎯 ===== NO ABSORPTION SCENARIO =====`);
            console.log(
                `📊 Context: Normal trading without institutional absorption`
            );
            ...
            for (const [i, trade] of trades.entries()) {
                ...
                console.log(
                    `    Price moved: ${trade.price !== basePrice ? "YES" : "NO"}`
                );
