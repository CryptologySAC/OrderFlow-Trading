...
const createFlowTraceLogger = (): ILogger => {
    ...
    return {
        info: vi.fn((msg, data) => {
            console.log(
                `[FLOW-INFO][${correlationId}] ${msg}`,
                data ? JSON.stringify(data, null, 2) : ""
            );
        }),
        warn: vi.fn((msg, data) => {
            console.log(
                `[FLOW-WARN][${correlationId}] ${msg}`,
                data ? JSON.stringify(data, null, 2) : ""
            );
        }),
        error: vi.fn((msg, data) => {
            console.log(
                `[FLOW-ERROR][${correlationId}] ${msg}`,
                data ? JSON.stringify(data, null, 2) : ""
            );
        }),
        debug: vi.fn((msg, data) => {
            console.log(
                `[FLOW-DEBUG][${correlationId}] ${msg}`,
                data ? JSON.stringify(data, null, 2) : ""
            );
        }),
    };
...
describe("AbsorptionDetector - Execution Flow Trace", () => {
    ...
    it(
        "should trace BaseDetector execution flow vs AbsorptionDetector override",
        { timeout: 10000 },
        () => {
            console.log(`\n🔬 ===== EXECUTION FLOW TRACE =====`);
            console.log(
                `🎯 Objective: Trace BaseDetector.onEnrichedTrade → addTrade → checkForSignal flow`
            );
            ...
            detector.on("signalCandidate", (data) => {
                signalEmitted = true;
                console.log(
                    `🎯 FLOW TRACE SIGNAL: ${data.side} at ${data.price}`
                );
            });
            ...
            return new Promise<void>((resolve) => {
                setTimeout(() => {
                    ...
                    const errorCallCount = (mockLogger.error as any).mock.calls
                        .length;
                    const debugCallCount = (mockLogger.debug as any).mock.calls
                        .length;
                    ...
                    if (totalLogCalls === 0) {
                        ...
                        console.log(
                            `  This confirms that AbsorptionDetector internal methods are not executing`
                        );
                        console.log(`  Possible causes:`);
                        console.log(
                            `    1. super.onEnrichedTrade() is not calling addTrade()`
                        );
                        console.log(
                            `    2. addTrade() is not calling checkForSignal()`
                        );
                        console.log(
                            `    3. checkForSignal() override is not calling internal analysis methods`
                        );
                        console.log(
                            `    4. Internal analysis methods return early without logging`
                        );
                    } else {
                        console.log(
                        ...
                        console.log(
                            `  Internal methods are executing but may be blocked by conditions`
                        );
                    }
                    ...
                    console.log(
                        `  OrderBook.getBestBid calls: ${mockOrderBook.getBestBid.mock.calls.length}`
                    );
                    console.log(
                        `  OrderBook.getBestAsk calls: ${mockOrderBook.getBestAsk.mock.calls.length}`
                    );
                    console.log(
                        `  OrderBook.getDepth calls: ${mockOrderBook.getDepth.mock.calls.length}`
                    );
                    console.log(
                        `  SpoofingDetector.isSpoofed calls: ${mockSpoofingDetector.isSpoofed.mock.calls.length}`
                    );

                    console.log(`\n🔧 NEXT DIAGNOSTIC STEPS:`);
                    if (totalLogCalls === 0) {
                        console.log(
                            `  1. Verify AbsorptionDetector.checkForSignal() is being called`
                        );
                        console.log(
                            `  2. Check if analyzeZoneForAbsorption() is reached`
                        );
                        console.log(
                            `  3. Investigate zone aggregation (this.zoneAgg) population`
                        );
                        console.log(
                            `  4. Verify parent class BaseDetector.addTrade() execution`
                        );
                    }
    ...
    it(
        "should verify zone aggregation and bucket creation",
        { timeout: 5000 },
        () => {
            ...
            console.log(
                `  Expected zone calculation: consistent for same price`
            );
            ...
            for (let i = 0; i < 3; i++) {
                ...
                console.log(
                    `  Trade ${i + 1}: SELL ${event.quantity} at ${basePrice} (${event.tradeId})`
                );
