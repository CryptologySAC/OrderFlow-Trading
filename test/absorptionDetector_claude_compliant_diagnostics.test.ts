    ...
    return {
        info: vi.fn((msg, data) => {
            if (msg.includes("AbsorptionDetector") || 
                ...
                console.log(`[INFO][${correlationId}] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
            }
        }),
        warn: vi.fn((msg, data) => {
            console.log(`[WARN][${correlationId}] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
        }),
        error: vi.fn((msg, data) => {
            console.log(`[ERROR][${correlationId}] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
        }),
        debug: vi.fn((msg, data) => {
            if (msg.includes("AbsorptionDetector")) {
                console.log(`[DEBUG][${correlationId}] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
            }
        }),
    };
};
...
describe("AbsorptionDetector - CLAUDE.md Compliant Diagnostics", () => {
    ...
    it("should diagnose exact blocking point with comprehensive tracing", { timeout: 15000 }, () => {
        console.log(`\n🔬 ===== CLAUDE.MD COMPLIANT DIAGNOSTICS =====`);
        console.log(`📊 Correlation ID: ${correlationId}`);
        console.log(`🎯 Objective: Identify exact blocking point in detection chain`);
        ...
        console.log(`\n📊 Diagnostic Settings (all permissive):`, {
            ...
            priceEfficiencyThreshold: diagnosticSettings.priceEfficiencyThreshold,
        ...
        institutionalSellingTrades.forEach((trade, i) => {
            ...
            if (trade.buyerIsMaker) {
                ...
                console.log(`  Trade ${i + 1}: SELL ${trade.quantity} at ${basePrice} (${Math.abs(trade.timeOffset / 1000)}s ago)`);
            } else {
                totalBuyVolume += trade.quantity;
                console.log(`  Trade ${i + 1}: BUY ${trade.quantity} at ${basePrice} (${Math.abs(trade.timeOffset / 1000)}s ago)`);
            }
            
            console.log(`    Processing trade with correlation ID: ${event.tradeId}`);
            detector.onEnrichedTrade(event);
        });

        const sellRatio = (totalSellVolume / (totalSellVolume + totalBuyVolume) * 100);
        console.log(`\n📊 Volume Summary:`);
        ...
        console.log(`  Expected Signal: BUY (institutions absorbing selling pressure)`);
            ...
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    ...
                    if (signalData.side.toUpperCase() === "BUY") {
                        ...
                        console.log(`  ❌ WRONG DIRECTION: Expected BUY, got ${signalData.side}`);
                    }
                } else {
                    console.log(`\n❌ BLOCKED: No signal generated with ultra permissive settings`);
                    
                    if (infoCallCount === 0 && warnCallCount === 0) {
                        console.log(`  🚨 CRITICAL: No detector methods executed at all`);
                        console.log(`  Issue: Detection chain not running (execution flow failure)`);
                    } else {
                        console.log(`  🔍 ANALYSIS: Detection methods ran but were blocked by conditions`);
                        console.log(`  Check logged conditions to identify specific blocking point`);
                    }
                }
                
                console.log(`\n📊 Next Steps Based on Results:`);
                if (!signalEmitted) {
                    console.log(`  1. Investigate zone aggregation and history population`);
                    console.log(`  2. Check passive volume calculation methods`);
                    console.log(`  3. Verify price efficiency calculation requirements`);
                    console.log(`  4. Analyze absorption condition validation logic`);
                }
    ...
    it("should trace zone history and passive volume population", { timeout: 10000 }, () => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            console.log(`  Processing trade ${i + 1}: SELL ${event.quantity} at ${testPrice}`);
            detector.onEnrichedTrade(event);
        }
