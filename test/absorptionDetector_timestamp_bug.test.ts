...
describe("AbsorptionDetector - Timestamp Bug Investigation", () => {
    ...
    it("should demonstrate timestamp bug causing wrong signal direction", { timeout: 10000 }, () => {
        console.log(`\n🚨 ===== TIMESTAMP BUG INVESTIGATION =====`);
        console.log(`🎯 HYPOTHESIS: getDominantAggressiveSide uses Date.now() instead of trade timestamps`);
        ...
        const now = Date.now();
        console.log(`\n📊 Current time: ${now}`);
        console.log(`📊 dominantSideAnalysisWindowMs: ${settings.dominantSideAnalysisWindowMs}`);
        console.log(`📊 Cutoff time: ${now - settings.dominantSideAnalysisWindowMs!}`);
        ...
        problemTrades.forEach((trade, i) => {
            ...
            const withinWindow = tradeTimestamp >= (now - settings.dominantSideAnalysisWindowMs!);
            
            console.log(`  Trade ${i + 1}: SELL ${trade.quantity} at ${tradeTimestamp} (${Math.abs(trade.offset / 1000)}s ago) - ${withinWindow ? 'WITHIN' : 'OUTSIDE'} window`);
        ...
        console.log(`  Trades Outside Window: ${problemTrades.length - withinWindowCount}`);
        console.log(`  Total Sell Volume: ${totalSellVolume}`);
        console.log(`  Expected: BUY signal (bid absorption of selling pressure)`);
        
        console.log(`\n🚨 BUG EFFECT:`);
        console.log(`  - getDominantAggressiveSide() filters trades by Date.now() - windowMs`);
        console.log(`  - Test trades with old timestamps get filtered out`);
        console.log(`  - Method falls back to last few trades (inconsistent sample)`);
            ...
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    ...
                    if (signalData.side === "buy") {
                        console.log(`  ✅ CORRECT: BUY signal for selling pressure`);
                        console.log(`  The timestamp filtering worked correctly this time`);
                    } else {
                        console.log(`  ❌ WRONG: ${signalData.side.toUpperCase()} signal for selling pressure`);
                        console.log(`  🚨 CONFIRMED: Timestamp bug causing wrong signal direction!`);
                        console.log(`\n💡 SOLUTION NEEDED:`);
                        console.log(`    Change line 876 from:`);
                        console.log(`    const cutoff = Date.now() - this.dominantSideAnalysisWindowMs;`);
                        console.log(`    To:`);
                        console.log(`    const latestTimestamp = Math.max(...trades.map(t => t.timestamp));`);
                        console.log(`    const cutoff = latestTimestamp - this.dominantSideAnalysisWindowMs;`);
                    }
                } else {
                    console.log(`  No signal generated - but timestamp bug still exists`);
                }
    ...
    it("should test with CURRENT timestamps (production scenario)", { timeout: 5000 }, () => {
        console.log(`\n🔬 ===== CURRENT TIMESTAMP TEST =====`);
        console.log(`🎯 Testing with current timestamps (how production works)`);
        ...
        currentTrades.forEach((trade, i) => {
            ...
            console.log(`  Trade ${i + 1}: SELL ${trade.quantity} (${Math.abs(trade.offset / 1000)}s ago)`);
            detector.onEnrichedTrade(event);
        });
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    ...
                    console.log(`  This shows how the detector behaves with proper timestamps`);
                }
