...
const createMockLogger = (): ILogger => ({
    info: vi.fn((msg, data) => console.log(`[INFO] ${msg}`, JSON.stringify(data, null, 2))),
    warn: vi.fn((msg, data) => console.log(`[WARN] ${msg}`, JSON.stringify(data, null, 2))),
    error: vi.fn((msg, data) => console.log(`[ERROR] ${msg}`, JSON.stringify(data, null, 2))),
    debug: vi.fn((msg, data) => console.log(`[DEBUG] ${msg}`, JSON.stringify(data, null, 2))),
});
...
describe("AbsorptionDetector - MATHEMATICAL EVIDENCE for Signal Direction", () => {
    ...
    describe("🧮 MATHEMATICAL SCENARIO BREAKDOWN", () => {
        ...
        const mathematicalScenarios: MathematicalScenario[] = [
            ...
            // 📊 SCENARIO 1: Pure Aggressive Buying
            {
                ...
                mathLogic: "buyVolume=8×qty > sellVolume=0 → dominantSide='buy' → absorbingSide='ask' → signal='sell'",
            ...
            // 📊 SCENARIO 2: Pure Aggressive Selling  
            {
                ...
                mathLogic: "sellVolume=8×qty > buyVolume=0 → dominantSide='sell' → absorbingSide='bid' → signal='buy'",
                setup: (baseTime: number, basePrice: number) => {
                    const events: EnrichedTradeEvent[] = [];
            ...
            // 📊 SCENARIO 3: Mixed but Buy-Dominant
            {
                ...
                mathLogic: "buyVolume=6×qty > sellVolume=2×qty → dominantSide='buy' → absorbingSide='ask' → signal='sell'",
                setup: (baseTime: number, basePrice: number) => {
                    const events: EnrichedTradeEvent[] = [];
            ...
            // 📊 SCENARIO 4: Mixed but Sell-Dominant
            {
                ...
                mathLogic: "sellVolume=6×qty > buyVolume=2×qty → dominantSide='sell' → absorbingSide='bid' → signal='buy'",
                setup: (baseTime: number, basePrice: number) => {
                    const events: EnrichedTradeEvent[] = [];
            ...
            // 📊 SCENARIO 5: Exactly Balanced (Edge Case)
            {
                ...
                description: "4 aggressive BUY + 4 aggressive SELL trades (same volume)",
                expectedDominantAggressiveSide: "sell", // tie-breaker: returns "sell" in current logic
                expectedAbsorbingSide: "bid",
                expectedSignalSide: "buy",
                mathLogic: "buyVolume=sellVolume → tie-breaker returns 'sell' → absorbingSide='bid' → signal='buy'",
                setup: (baseTime: number, basePrice: number) => {
                    const events: EnrichedTradeEvent[] = [];
        ...
        mathematicalScenarios.forEach((scenario) => {
            it(`should provide mathematical evidence for: ${scenario.name}`, { timeout: 15000 }, () => {
                console.log(`\n🧮 ========================= MATHEMATICAL EVIDENCE =========================`);
                console.log(`📊 SCENARIO: ${scenario.name}`);
                console.log(`📝 DESCRIPTION: ${scenario.description}`);
                console.log(`🧠 MATH LOGIC: ${scenario.mathLogic}`);
                console.log(`\n📐 EXPECTED CALCULATIONS:`);
                console.log(`  • Dominant Aggressive Side: ${scenario.expectedDominantAggressiveSide}`);
                console.log(`  • Absorbing Side: ${scenario.expectedAbsorbingSide}`);
                console.log(`  • Final Signal Side: ${scenario.expectedSignalSide}`);
                console.log(`\n🔬 PROCESSING TRADES...`);
                ...
                events.forEach((event, i) => {
                    if (event.buyerIsMaker) {
                        ...
                        console.log(`  Trade ${i + 1}: SELL ${event.quantity} (buyerIsMaker=true) → aggressive sell volume`);
                    } else {
                        // buyerIsMaker=false → aggressive buy
                        totalBuyVolume += event.quantity;
                        console.log(`  Trade ${i + 1}: BUY ${event.quantity} (buyerIsMaker=false) → aggressive buy volume`);
                    }
                ...
                console.log(`\n📈 VOLUME CALCULATIONS:`);
                console.log(`  • Total Aggressive Buy Volume: ${totalBuyVolume}`);
                console.log(`  • Total Aggressive Sell Volume: ${totalSellVolume}`);
                console.log(`  • Dominant Side: ${totalBuyVolume > totalSellVolume ? "buy" : "sell"}`);
                console.log(`  • Expected Absorbing Side: ${totalBuyVolume > totalSellVolume ? "ask" : "bid"} (opposite of dominant)`);
                console.log(`  • Expected Signal: ${totalBuyVolume > totalSellVolume ? "sell" : "buy"} (follow absorbing side)`);
                ...
                return new Promise<void>((resolve) => {
                    setTimeout(() => {
                        ...
                        if (signalEmitted) {
                            console.log(`✅ Signal Generated: ${actualSignal}`);
                            console.log(`📊 Internal Calculations:`, {
                                absorbingSide: calculationData?.absorbingSide,
                                aggressiveSide: calculationData?.aggressiveSide,
                                signalInterpretation: calculationData?.signalInterpretation,
                                absorptionType: calculationData?.absorptionType
                            });
                            
                            // Mathematical verification
                            const expectedSignal = scenario.expectedSignalSide;
                            const matchesExpected = actualSignal === expectedSignal;
                            
                            console.log(`\n🎯 RESULT ANALYSIS:`);
                            console.log(`  Expected: ${expectedSignal}`);
                            console.log(`  Actual: ${actualSignal}`);
                            console.log(`  Match: ${matchesExpected ? "✅ CORRECT" : "❌ WRONG"}`);
                            
                            if (!matchesExpected) {
                                console.log(`\n🚨 MATHEMATICAL ERROR DETECTED!`);
                                console.log(`  This indicates a bug in the calculation logic`);
                                console.log(`  Expected math: ${scenario.mathLogic}`);
                                console.log(`  Actual result violates mathematical expectations`);
                            }
                            
                        } else {
                            console.log(`⚠️ No Signal Generated`);
                            console.log(`  This may indicate threshold/filtering issues`);
                            console.log(`  Or the scenario didn't meet detection criteria`);
                        }
    ...
    describe("🔍 EDGE CASE MATHEMATICAL ANALYSIS", () => {
        ...
        it("should show tie-breaker logic for equal volumes", { timeout: 10000 }, () => {
            ...
            const events = [
                ...
                createMathematicalTestEvent(basePrice, 100, baseTime, false, 1000, 1000),
                createMathematicalTestEvent(basePrice, 100, baseTime + 5000, false, 1000, 1000),
                // 2 aggressive sells: 200 total volume
                createMathematicalTestEvent(basePrice, 100, baseTime + 10000, true, 1000, 1000),
                createMathematicalTestEvent(basePrice, 100, baseTime + 15000, true, 1000, 1000),
            ];
            ...
            return new Promise<void>((resolve) => {
                setTimeout(() => {
                    ...
                    console.log(`  Tie-breaker chose: ${actualSignal || "None"}`);
                    resolve();
                }, 100);
            });
        });
