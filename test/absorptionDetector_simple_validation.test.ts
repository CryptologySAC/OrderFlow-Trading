...
describe("AbsorptionDetector - Simple Validation", () => {
    ...
    it(
        "should detect simple selling absorption pattern",
        { timeout: 5000 },
        async () => {
            ...
            for (const [i, trade] of trades.entries()) {
                ...
                console.log(
                    `  Trade ${i + 1}: SELL ${trade.quantity} at ${trade.price}`
                );
            ...
            if (signals.length > 0) {
                ...
                console.log(
                    `  Last signal: ${lastSignal.side} at ${lastSignal.price}`
                );
    ...
    it(
        "should detect simple buying absorption pattern",
        { timeout: 5000 },
        async () => {
            ...
            for (const [i, trade] of trades.entries()) {
                ...
                console.log(
                    `  Trade ${i + 1}: BUY ${trade.quantity} at ${trade.price}`
                );
                detector.onEnrichedTrade(event);
            }
