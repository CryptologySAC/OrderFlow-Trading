...
describe("AbsorptionDetector - CORRECT Behavior Specification", () => {
    ...
    describe("🚨 Phase 1: Market Extremes (Critical Bug Detection)", () => {
        const marketExtremeScenarios: AbsorptionTestScenario[] = [
            // 🔴 THE IMPOSSIBLE SCENARIO: Your SELL signal at market bottom
            {
                ...
                description:
                    "Heavy selling pressure at market bottom - institutions absorbing on bid side",
                marketContext: "market_bottom",
                expectedSignal: "buy", // ✅ CORRECT: Institutions buying, should signal buy
                expectedLogic:
                    "Heavy selling pressure → bid side absorbing → institutions buying → buy signal",
                mathProof:
                    "90% buyerIsMaker=true → 90% sellVolume → dominantSide='sell' → absorbingSide='bid' → signal='buy'",
                trades: [
...

            {
                ...
                description:
                    "Heavy buying pressure at market top - institutions absorbing on ask side",
                marketContext: "market_top",
                expectedSignal: "sell", // ✅ CORRECT: Institutions selling, should signal sell
                expectedLogic:
                    "Heavy buying pressure → ask side absorbing → institutions selling → sell signal",
                mathProof:
                    "90% buyerIsMaker=false → 90% buyVolume → dominantSide='buy' → absorbingSide='ask' → signal='sell'",
                trades: [
...

            {
                ...
                description:
                    "Panic selling absorbed by institutional buying (your exact scenario)",
                marketContext: "market_bottom",
                expectedSignal: "buy", // ✅ NEVER SELL at market bottom!
                expectedLogic:
                    "Panic selling → institutional bid absorption → BUY signal",
                mathProof:
                    "95% sellVolume → dominantSide='sell' → absorbingSide='bid' → signal='BUY'",
                trades: [
                    // 19 panic sells
...

            {
                ...
                description:
                    "Retail FOMO buying absorbed by institutional selling",
                marketContext: "market_top",
                expectedSignal: "sell", // ✅ NEVER BUY at market top!
                expectedLogic:
                    "FOMO buying → institutional ask absorption → SELL signal",
                mathProof:
                    "95% buyVolume → dominantSide='buy' → absorbingSide='ask' → signal='SELL'",
                trades: [
                    // 19 FOMO buys
                    ...Array.from({ length: 19 }, (_, i) => ({
                        buyerIsMaker: false,
        ...
        marketExtremeScenarios.forEach((scenario) => {
            it(
                ...
                () => {
                    ...
                    console.log(
                        `📊 Market Context: ${scenario.marketContext.toUpperCase()}`
                    );
                    console.log(
                        `🎯 Expected Signal: ${scenario.expectedSignal || "None"}`
                    );
                    ...
                    scenario.trades.forEach((trade, i) => {
                        ...
                        if (scenario.expectedSignal === "buy") {
                            ...
                            const isLaterTrade =
                                i >= Math.floor(scenario.trades.length / 2);
                            passiveBid = isLaterTrade
                                ? 5000 + i * 100
                                : 5000 - i * 50; // Refill pattern
                            passiveAsk = 4500 - i * 20;
                        } else if (scenario.expectedSignal === "sell") {
                            // For SELL signals: ask liquidity refills during buying
                            const isLaterTrade =
                                i >= Math.floor(scenario.trades.length / 2);
                            passiveAsk = isLaterTrade
                                ? 4500 + i * 100
                                : 4500 - i * 50; // Refill pattern
                        ...
                        if (trade.buyerIsMaker) {
                            ...
                            console.log(
                                `  ${i + 1}. SELL ${trade.quantity} (aggressive seller, buyerIsMaker=true)`
                            );
                        } else {
                            totalBuyVolume += trade.quantity; // Buyer was aggressive
                            console.log(
                                `  ${i + 1}. BUY ${trade.quantity} (aggressive buyer, buyerIsMaker=false)`
                            );
                        }
                    ...
                    console.log(`\n📈 VOLUME ANALYSIS:`);
                    console.log(
                        `  Total Sell Volume (aggressive sellers): ${totalSellVolume}`
                    );
                    console.log(
                        `  Total Buy Volume (aggressive buyers): ${totalBuyVolume}`
                    );
                    console.log(
                        `  Dominant Side: ${totalSellVolume > totalBuyVolume ? "SELL" : "BUY"}`
                    );
                    console.log(
                        `  Sell Ratio: ${((totalSellVolume / (totalSellVolume + totalBuyVolume)) * 100).toFixed(1)}%`
                    );
                    console.log(
                        `  Buy Ratio: ${((totalBuyVolume / (totalSellVolume + totalBuyVolume)) * 100).toFixed(1)}%`
                    );
                    ...
                    return new Promise<void>((resolve) => {
                        setTimeout(() => {
                            ...
                            if (scenario.expectedSignal === null) {
                                ...
                                if (signalEmitted) {
                                    console.log(
                                        `❌ SPECIFICATION VIOLATION: Expected no signal but got ${actualSignal}`
                                    );
                                    console.log(
                                        `   This indicates incorrect threshold or filtering logic`
                                    );
                                } else {
                                    console.log(
                                        `✅ SPECIFICATION COMPLIANT: Correctly generated no signal`
                                    );
                                }
                            } else {
                                ...
                                if (signalEmitted) {
                                    const isCorrect =
                                        actualSignal?.toLowerCase() ===
                                        scenario.expectedSignal;
                                    if (isCorrect) {
                                        console.log(
                                            `✅ SPECIFICATION COMPLIANT: ${actualSignal} matches expected ${scenario.expectedSignal}`
                                        );
                                    } else {
                                        console.log(
                                            `❌ SPECIFICATION VIOLATION: Expected ${scenario.expectedSignal}, got ${actualSignal}`
                                        );
                                        console.log(
                                            `   🚨 CRITICAL BUG: Signal direction logic is WRONG`
                                        );
                                        console.log(
                                            `   Market Context: ${scenario.marketContext}`
                                        );
                                        console.log(
                                            `   This violates institutional trading principles!`
                                        );
                                        ...
                                        if (
                                            ...
                                            console.log(
                                                `   🎯 SMOKING GUN: SELL signal at market bottom is IMPOSSIBLE!`
                                            );
                                        }
                                        if (
                                            scenario.marketContext ===
                                            ...
                                            console.log(
                                                `   🎯 SMOKING GUN: BUY signal at market top is IMPOSSIBLE!`
                                            );
                                        }
                                    }

                                    // ✅ CLAUDE.md COMPLIANT: Tests detect bugs, never adjust to pass buggy code
                                    expect(actualSignal?.toLowerCase()).toBe(
                                        scenario.expectedSignal
                                    );
                                } else {
                                    console.log(
                                        `⚠️ SPECIFICATION ISSUE: Expected ${scenario.expectedSignal} but no signal generated`
                                    );
                                    console.log(
        ...
        const mixedRatioScenarios: AbsorptionTestScenario[] = [
            {
                ...
                description:
                    "70% selling pressure absorbed by institutional bids",
                marketContext: "sideways",
                expectedSignal: "buy",
                expectedLogic:
                    "Dominant selling pressure → bid absorption → BUY signal",
                mathProof:
                    "70% sellVolume → dominantSide='sell' → absorbingSide='bid' → signal='BUY'",
                trades: [
                    // 7 sells
...

            {
                ...
                description:
                    "70% buying pressure absorbed by institutional asks",
                marketContext: "sideways",
                expectedSignal: "sell",
                expectedLogic:
                    "Dominant buying pressure → ask absorption → SELL signal",
                mathProof:
                    "70% buyVolume → dominantSide='buy' → absorbingSide='ask' → signal='SELL'",
                trades: [
                    // 7 buys
                    ...Array.from({ length: 7 }, (_, i) => ({
                        buyerIsMaker: false,
...

            {
                ...
                description:
                    "Moderate selling pressure - test detection threshold",
                marketContext: "sideways",
                expectedSignal: "buy",
                expectedLogic:
                    "Moderate selling pressure → bid absorption → BUY signal",
                mathProof:
                    "60% sellVolume → dominantSide='sell' → absorbingSide='bid' → signal='BUY'",
                trades: [
                    // 6 sells
...

            {
                ...
                expectedLogic:
                    "Balanced pressure → no clear absorption → no signal",
                mathProof:
                    "50% each → no dominant side → no absorption detected",
                trades: [
            ...
            it(
                ...
                () => {
                    ...
                    scenario.trades.forEach((trade, i) => {
                        ...
                        if (trade.buyerIsMaker) sellCount++;
                        else buyCount++;
                        detector.onEnrichedTrade(event);
                    });

                    console.log(
                        `  Executed: ${sellCount} sells, ${buyCount} buys`
                    );
                    ...
                    return new Promise<void>((resolve) => {
                        setTimeout(() => {
                            ...
                            const expected =
                                scenario.expectedSignal || "No Signal";
                            console.log(
    ...
    describe("⚡ Phase 3: Volume Intensity & Edge Cases", () => {
        it(
            "should handle high volume absorption correctly",
            { timeout: 10000 },
            () => {
                ...
                return new Promise<void>((resolve) => {
                    setTimeout(() => {
                        console.log(
                            `  High volume result: ${actualSignal || "No Signal"}`
                        );
        ...
        it(
            "should reject low volume scenarios correctly",
            { timeout: 10000 },
            () => {
                ...
                return new Promise<void>((resolve) => {
                    setTimeout(() => {
                        console.log(
                            `  Low volume result: ${signalEmitted ? "Signal Generated" : "No Signal"}`
                        );
    ...
    describe("🎯 Phase 4: Critical Bug Validation", () => {
        it(
            "should NEVER generate SELL signal at market bottom",
            { timeout: 10000 },
            () => {
                console.log(
                ...
                detector.on("signalCandidate", (data) => {
                    ...
                    if (data.side.toLowerCase() === "sell") {
                        ...
                        console.log(
                            `🚨 CRITICAL BUG: SELL signal at market bottom!`
                        );
                    }
                });
                ...
                return new Promise<void>((resolve) => {
                    setTimeout(() => {
                        console.log(
                            `  Market bottom result: ${actualSignal || "No Signal"}`
                        );
        ...
        it(
            "should NEVER generate BUY signal at market top",
            { timeout: 10000 },
            () => {
                ...
                detector.on("signalCandidate", (data) => {
                    ...
                    if (data.side.toLowerCase() === "buy") {
                        ...
                        console.log(
                            `🚨 CRITICAL BUG: BUY signal at market top!`
                        );
                    }
                });
                ...
                return new Promise<void>((resolve) => {
                    setTimeout(() => {
                        console.log(
                            `  Market top result: ${actualSignal || "No Signal"}`
                        );
