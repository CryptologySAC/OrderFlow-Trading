...
describe("AbsorptionDetector - Signal Trace", () => {
    ...
    it("should trace complete signal generation flow", { timeout: 5000 }, () => {
        ...
        const calculateAbsorptionScoreSpy = vi.spyOn(detector as any, 'calculateAbsorptionScore');
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                if (getAbsorbingSideForZoneSpy.mock.results.length > 0) {
                    const results = getAbsorbingSideForZoneSpy.mock.results.map(r => r.value);
                ...
                if (checkCooldownSpy.mock.results.length > 0) {
                    const results = checkCooldownSpy.mock.results.map(r => r.value);
                    console.log(`   Returns: ${results.join(', ')}`);
                }
                ...
                if (checkAbsorptionConditionsSpy.mock.results.length > 0) {
                    const results = checkAbsorptionConditionsSpy.mock.results.map(r => r.value);
                ...
                if (calculateAbsorptionScoreSpy.mock.results.length > 0) {
                    const results = calculateAbsorptionScoreSpy.mock.results.map(r => r.value);
                    console.log(`   Returns: ${results.join(', ')}`);
                }
                ...
                if (!signalEmitted) {
                    ...
                    if (checkCooldownSpy.mock.results.some(r => r.value === false)) {
                        console.log(`   ❌ Cooldown blocking signals`);
                    }
                    
                    if (checkAbsorptionConditionsSpy.mock.calls.length === 0) {
                        console.log(`   ❌ Never reached absorption condition check`);
                        console.log(`      Likely blocked earlier in analyzeZoneForAbsorption`);
                    } else if (checkAbsorptionConditionsSpy.mock.results.some(r => !r.value)) {
                        console.log(`   ❌ Absorption conditions not met`);
                    }
                    
                    if (calculateAbsorptionScoreSpy.mock.calls.length === 0) {
                        console.log(`   ❌ Never calculated absorption score`);
                    }
                }
