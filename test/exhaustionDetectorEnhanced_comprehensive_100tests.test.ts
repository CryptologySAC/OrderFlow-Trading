...
describe("ExhaustionDetectorEnhanced - Comprehensive 100 Test Suite (FIXED)", () => {
    ...
    const createMockPreprocessor = (): IOrderflowPreprocessor => ({
        ...
        findZonesNearPrice: vi.fn((zones, price, maxDistance) => {
            ...
            return zones.filter((zone: ZoneSnapshot) => 
    ...
    function createZoneSnapshot(
        ...
        return {
            ...
            tradeCount: Math.max(1, Math.floor(aggressiveVolume / 5)) * multiplier,
            timespan: 60000,
    ...
    function createStandardizedZoneData(
        price: number,
        config: {
            zones5?: Array<{ aggressiveVol: number; passiveVol: number; priceOffset: number }>;
            zones10?: Array<{ aggressiveVol: number; passiveVol: number; priceOffset: number }>;
            zones20?: Array<{ aggressiveVol: number; passiveVol: number; priceOffset: number }>;
        } = {}
    ): StandardZoneData {
        ...
        return {
            ...
            zones5Tick: defaultZones5.map(z => 
                createZoneSnapshot(price + z.priceOffset, z.aggressiveVol, z.passiveVol, 1)
            ),
            zones10Tick: defaultZones10.map(z => 
                createZoneSnapshot(price + z.priceOffset, z.aggressiveVol, z.passiveVol, 1) // Use 1x, not 2x
            ),
            zones20Tick: defaultZones20.map(z => 
                createZoneSnapshot(price + z.priceOffset, z.aggressiveVol, z.passiveVol, 1) // Use 1x, not 4x
            ),
        };
    }
    ...
    function createTradeEvent(
        ...
        return {
            ...
            marketPrice: price,
            zoneData: zoneData !== undefined ? zoneData : createStandardizedZoneData(price), // Only create default if not explicitly undefined
            bookMetrics: {
                spread: 0.01,
                spreadBps: 1,
                midPrice: price,
    ...
    describe("Core Exhaustion Detection", () => {
        it("should detect exhaustion when aggressive volume exceeds threshold with high ratio", () => {
            const zoneData = createStandardizedZoneData(87.50, {
                zones5: [{ aggressiveVol: 50, passiveVol: 5, priceOffset: 0 }] // Working ratio from debug: 0.909
            });
            const trade = createTradeEvent(87.50, 25, false, zoneData); // Above minAggVolume (15)
        ...
        it("should NOT detect exhaustion when volume below minimum threshold", () => {
            ...
            const trade = createTradeEvent(87.50, 5, false, zoneData); // Below minAggVolume (15)
        ...
        it("should detect BUY exhaustion with high buying pressure", () => {
            ...
            const trade = createTradeEvent(87.50, 25, false, zoneData); // buyerIsMaker=false = aggressive buy
        ...
        it("should detect SELL exhaustion with high selling pressure", () => {
            ...
            const trade = createTradeEvent(87.50, 25, true, zoneData); // buyerIsMaker=true = aggressive sell
        ...
        it("should require multiple exhausted zones for high confidence", () => {
            ...
            const trade = createTradeEvent(87.50, 25, false, zoneData);
        ...
        it("should handle zones with zero volume gracefully", () => {
            ...
            const trade = createTradeEvent(87.50, 15, false, zoneData);
        ...
        it("should filter zones by price proximity using maxDistance", () => {
            ...
            const trade = createTradeEvent(87.50, 18, false, zoneData);
        ...
        it("should apply confidence threshold correctly", () => {
            const zoneData = createStandardizedZoneData(87.50, {
                ...
                zones10: [{ aggressiveVol: 18, passiveVol: 25, priceOffset: 0 }], // Lower ratio 
                zones20: [{ aggressiveVol: 20, passiveVol: 30, priceOffset: 0 }]  // Lower ratio
            });
            const trade = createTradeEvent(87.50, 15, false, zoneData);
        ...
        it("should handle extreme volume ratios correctly", () => {
            ...
            const trade = createTradeEvent(87.50, 30, false, zoneData);
        ...
        it("should distinguish between buy and sell exhaustion", () => {
            ...
            const sellZoneData = createStandardizedZoneData(87.50, {
                zones5: [{ aggressiveVol: 50, passiveVol: 5, priceOffset: 0 }],
                zones10: [{ aggressiveVol: 75, passiveVol: 7.5, priceOffset: 0 }],
                zones20: [{ aggressiveVol: 100, passiveVol: 10, priceOffset: 0 }]
            });
            // Override all zones to have more sell volume (30% buy, 70% sell)
            [sellZoneData.zones5Tick[0], sellZoneData.zones10Tick[0], sellZoneData.zones20Tick[0]].forEach(zone => {
                if (zone) {
            ...
            const sellTrade = createTradeEvent(87.50, 25, true, sellZoneData);
        ...
        it("should handle tick-compliant price movements", () => {
            const zoneData = createStandardizedZoneData(89.01, { // Valid tick price
        ...
        it("should validate exhaustion score meets minimum threshold", () => {
            ...
            const trade = createTradeEvent(87.50, 20, false, zoneData);
        ...
        it("should detect institutional volume exhaustion", () => {
            ...
            const trade = createTradeEvent(87.50, 50, false, zoneData);
    ...
    describe("Zone Confluence Analysis", () => {
        ...
        it("should distinguish between weak and strong confluence", () => {
            ...
            const weakZoneData = createStandardizedZoneData(87.50, {
                zones5: [{ aggressiveVol: 16, passiveVol: 20, priceOffset: 0 }], // Low ratio, shouldn't trigger
                zones10: [{ aggressiveVol: 18, passiveVol: 25, priceOffset: 0 }],
                zones20: [{ aggressiveVol: 20, passiveVol: 30, priceOffset: 0 }]
            });
            const weakTrade = createTradeEvent(87.50, 15, false, weakZoneData);

            detector.onEnrichedTrade(weakTrade);
            const weakConfidence = signalSpy.mock.calls.length > 0 ? 
                (signalSpy.mock.calls[0][0] as SignalCandidate).confidence : 0;

            signalSpy.mockClear();

            // Test strong confluence with high exhaustion ratios
            const strongZoneData = createStandardizedZoneData(87.50, {
                zones5: [
                    { aggressiveVol: 50, passiveVol: 5, priceOffset: 0 },
                    { aggressiveVol: 45, passiveVol: 4, priceOffset: 0.01 }
                ],
            ...
            const strongTrade = createTradeEvent(87.50, 25, false, strongZoneData);

            detector.onEnrichedTrade(strongTrade);
            const strongConfidence = signalSpy.mock.calls.length > 0 ? 
                (signalSpy.mock.calls[0][0] as SignalCandidate).confidence : 0;
        ...
        it("should handle asymmetric zone distributions", () => {
            const zoneData = createStandardizedZoneData(87.50, {
                ...
                zones10: [{ aggressiveVol: 30, passiveVol: 4, priceOffset: -0.01 }] // Zone below price
            });
            const trade = createTradeEvent(87.50, 20, false, zoneData);
    ...
    describe("Edge Cases and Error Handling", () => {
        ...
        it("should handle extremely small volume values", () => {
            const zoneData = createStandardizedZoneData(87.50, {
                zones5: [{ aggressiveVol: 0.001, passiveVol: 0.0005, priceOffset: 0 }]
            });
            const trade = createTradeEvent(87.50, 0.001, false, zoneData);
    ...
    describe("Realistic Market Scenarios", () => {
        ...
        it("should handle quiet market with minimal activity", () => {
            ...
            const trade = createTradeEvent(87.50, 1, false, zoneData);
