...
const createValidationLogger = (): ILogger => ({
    info: vi.fn((msg, data) => {
        if (msg.includes("CORRECTED ABSORPTION SIGNAL")) {
            console.log(`🎯 [VALIDATION SIGNAL] ${msg}`, JSON.stringify(data, null, 2));
        }
    }),
    warn: vi.fn((msg, data) => {
        if (msg.includes("No book data")) {
            console.log(`⚠️ [VALIDATION WARN] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
        }
    }),
    error: vi.fn(),
    debug: vi.fn(),
...
describe("AbsorptionDetector - CLAUDE.md Compliant Validation", () => {
    ...
    it("should validate CLAUDE.md fixes with proper data", { timeout: 10000 }, () => {
        ...
        console.log(`📊 Testing with expectedMovementScalingFactor: ${validationSettings.expectedMovementScalingFactor}`);
        ...
        detector.on("signalCandidate", (data) => {
            ...
            console.log(`  Signal ${signalCount}: ${data.side} at ${data.price}`);
        ...
        const validationTrades = [
            ...
            { buyerIsMaker: true, quantity: 80, bidVol: 3000, askVol: 2500, offset: -30000 },
            { buyerIsMaker: true, quantity: 90, bidVol: 2900, askVol: 2600, offset: -25000 },
            { buyerIsMaker: true, quantity: 100, bidVol: 2800, askVol: 2700, offset: -20000 },
            { buyerIsMaker: true, quantity: 110, bidVol: 2900, askVol: 2400, offset: -15000 }, // Bid refill
            { buyerIsMaker: true, quantity: 120, bidVol: 3100, askVol: 2200, offset: -10000 }, // Strong bid refill
            { buyerIsMaker: true, quantity: 100, bidVol: 3200, askVol: 2100, offset: -5000 },  // Absorption evident
        ];
        ...
        validationTrades.forEach((trade, i) => {
            ...
            console.log(`  Trade ${i + 1}: SELL ${trade.quantity} | Passive Bid: ${trade.bidVol} Ask: ${trade.askVol}`);
        ...
        console.log(`  Passive Bid Pattern: 3000→2800→2900→3100→3200 (absorption)`);
        console.log(`  Expected Signal: BUY (bid side absorbing selling pressure)`);
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    console.log(`  ✅ SUCCESS: CLAUDE.md fixes enable signal generation!`);
                    ...
                    console.log(`  ✅ SIGNAL DIRECTION CORRECT: BUY for selling pressure`);
                    console.log(`\n🎉 CLAUDE.md COMPLIANCE ACHIEVED:`);
                    console.log(`    - No fake passive volume data`);
                    console.log(`    - Proper tick-size movement validation`);
                    console.log(`    - Configurable scaling factor working`);
                    console.log(`    - Enhanced null handling working`);
                } else {
                    console.log(`  ❌ NEED INVESTIGATION: Still no signal despite fixes`);
                    console.log(`  Check if more conditions need adjustment`);
                }
    ...
    it("should reject invalid data per CLAUDE.md (no fake calculations)", { timeout: 5000 }, () => {
        ...
        invalidEvents.forEach((event, i) => {
            console.log(`  Trade ${i + 1}: SELL ${event.quantity} (passive: 0)`);
            detector.onEnrichedTrade(event);
        });
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                console.log(`  Signal Generated: ${signalEmitted}`);
                console.log(`  ✅ EXPECTED: No fake calculations with missing data`);
                console.log(`  This confirms CLAUDE.md principle: return null, not fake values`);
