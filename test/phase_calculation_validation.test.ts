...
describe('Phase Calculation Mathematical Validation', () => {
    describe('Core Mathematical Invariant', () => {
        it('NO signal movement can exceed phase movement', () => {
            ...
            const phase1 = {
                ...
                movement: Math.abs(FinancialMath.calculatePercentageChange(131.39, 129.59, 0)) / 100
            };
            ...
            const signalMovements = signals.map(s => 
                Math.abs(FinancialMath.calculatePercentageChange(s.entry, s.exit, 0)) / 100
            );
            ...
            expect(phase1.movement).toBeCloseTo(0.01370, 4);
            expect(signalMovements[0]).toBeCloseTo(0.01272, 4); // $131.26 signal
            expect(signalMovements[2]).toBeCloseTo(0.01370, 4); // $131.39 signal (equals phase)
        });
        ...
        it('Phase movement calculation must use correct start/end prices', () => {
            ...
            const correctPhaseMovement = Math.abs(
                FinancialMath.calculatePercentageChange(correctPhaseStart, finalLow, 0)
            ) / 100;
            
            // WRONG: Using lowest entry (would understate movement)
            const wrongPhaseStart = Math.min(...sellSignalEntries);
            const wrongPhaseMovement = Math.abs(
                FinancialMath.calculatePercentageChange(wrongPhaseStart, finalLow, 0)
            ) / 100;
            ...
            expect(correctPhaseMovement).toBeCloseTo(0.01370, 4);
            expect(wrongPhaseMovement).toBeCloseTo(0.01212, 4);
        });
        
        it('Each phase must be internally consistent', () => {
            const phases = [
                { start: 131.39, end: 129.59, expectedMovement: 0.01370 },
                { start: 130.89, end: 129.59, expectedMovement: 0.00993 },
                { start: 130.00, end: 129.59, expectedMovement: 0.00315 }
            ];
            
            for (const phase of phases) {
                const calculated = Math.abs(
                    FinancialMath.calculatePercentageChange(phase.start, phase.end, 0)
                ) / 100;
    ...
    describe('Validation Requirements', () => {
        it('should return null/0 for invalid calculations', () => {
            ...
            expect(FinancialMath.calculatePercentageChange(100, 0, -1)).toBe(-100);
            expect(FinancialMath.calculatePercentageChange(NaN, 100, 0)).toBe(0);
            expect(FinancialMath.calculatePercentageChange(100, NaN, 0)).toBe(0);
        });
        ...
        it('validates Phase 1 calculations are correct', () => {
            ...
            const phase1Data = {
                ...
                signals: [
                    ...
                    { entry: 131.39, movement: 0.01370 }
                ]
            };
            
            // All signals must be <= phase movement
            const allValid = phase1Data.signals.every(s => 
                s.movement <= phase1Data.phaseMovement + 0.00001 // Small tolerance for rounding
            );
            
            expect(allValid).toBe(true);
            
            // Signal at highest entry should equal phase movement
            const highestSignal = phase1Data.signals.find(s => s.entry === 131.39);
            expect(highestSignal?.movement).toBeCloseTo(phase1Data.phaseMovement, 4);
        });
        
        it('validates failed signals show correct percentages', () => {
            // Phase 3 failed signals
            const failedSignals = [
                { entry: 130.00, exit: 129.59, expectedMovement: 0.00315 },
                { entry: 129.77, exit: 129.64, expectedMovement: 0.00100 },
                { entry: 129.74, exit: 129.64, expectedMovement: 0.00077 }
            ];
            
            for (const signal of failedSignals) {
                const calculated = Math.abs(
                    FinancialMath.calculatePercentageChange(signal.entry, signal.exit, 0)
                ) / 100;
