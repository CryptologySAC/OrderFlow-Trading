...
import { EnrichedTradeEvent, HybridTradeEvent } from "../src/types/marketEvents.js";
    ...
    describe("1. Passive vs Aggressive Behavior Detection", () => {
        describe("buyerIsMaker Field Interpretation", () => {
            it("should correctly interpret buyerIsMaker=true as seller aggressive, buyer passive", () => {
                ...
                const aggressiveSide = detectorAny.getTradeSide(sellPressureTrade);
                const absorbingSide = detectorAny.getAbsorbingSide(sellPressureTrade);
                ...
                console.log("✅ buyerIsMaker=true correctly interpreted as sell pressure into bid support");
            });
            ...
            it("should correctly interpret buyerIsMaker=false as buyer aggressive, seller passive", () => {
                ...
                // Test trade side interpretation
                const aggressiveSide = detectorAny.getTradeSide(buyPressureTrade);
                const absorbingSide = detectorAny.getAbsorbingSide(buyPressureTrade);

                // Validate correct interpretation
                expect(aggressiveSide).toBe("buy"); // Buyer is aggressive
                expect(absorbingSide).toBe("sell"); // Seller is absorbing (passive)
                
                console.log("✅ buyerIsMaker=false correctly interpreted as buy pressure into ask resistance");
            });
        });

        describe("Dominant Flow Analysis", () => {
            it("should correctly identify dominant aggressive flow from trade patterns", () => {
                console.log("🧪 Testing dominant flow analysis for absorption detection");
                ...
                const dominantSide = detectorAny.getDominantAggressiveSide(buyDominantTrades);
                ...
                const dominantSide2 = detectorAny.getDominantAggressiveSide(sellDominantTrades);
                expect(dominantSide2).toBe("sell");
                
                console.log("✅ Dominant flow analysis correctly identifies aggressive side based on volume");
            });
        });
    });
    ...
    describe("2. Iceberg Detection and Hidden Liquidity", () => {
        describe("Passive Liquidity Refill Pattern Recognition", () => {
            it("should detect iceberg orders through systematic refill patterns", () => {
                ...
                const zoneHistory = detectorAny.zonePassiveHistory.get(testZone);
                expect(zoneHistory).toBeDefined();
                expect(zoneHistory.count()).toBeGreaterThan(5);
                
                console.log("✅ Iceberg detection recognizes refill patterns in passive liquidity");
            });
        });
    });
    ...
    describe("3. Volume Clustering and Price Stability Analysis", () => {
        describe("High Volume with Minimal Price Movement", () => {
            it("should detect absorption when volume clusters at a price level with minimal movement", () => {
                console.log("🧪 Testing volume clustering detection with price stability");
                ...
                const zoneHistory = detectorAny.zonePassiveHistory.get(clusterZone);
                expect(zoneHistory?.count()).toBeGreaterThanOrEqual(2); // Adjusted for realistic zone history
                
                console.log(`✅ Volume clustering detected: ${totalVolume} total volume with minimal price movement`);
            });
        });
    });
    ...
    describe("4. Delta Imbalance and Flow Analysis", () => {
        describe("Buy vs Sell Volume Imbalance Detection", () => {
            it("should detect significant delta imbalances that indicate absorption scenarios", () => {
                ...
                const dominantFlow = detectorAny.getDominantAggressiveSide(trades);
                ...
                console.log(`✅ Delta imbalance detected: ${(imbalanceRatio * 100).toFixed(1)}% buy, ${((1 - imbalanceRatio) * 100).toFixed(1)}% sell`);
            });
        });
    });
    ...
    describe("5. Context-Aware Absorption Analysis", () => {
        describe("Price Extreme Detection", () => {
            it("should enhance absorption analysis at price extremes for reversal detection", () => {
                console.log("🧪 Testing context-aware absorption at price extremes");
                ...
                const absorptionContext = detectorAny.calculateAbsorptionContext(
                    highPrice,
                ...
                console.log("✅ Context-aware analysis enhances absorption at price extremes");
            });
        });
    });
    ...
    describe("6. Adaptive Threshold Behavior", () => {
        describe("Market Condition Adaptation", () => {
            it("should adapt absorption thresholds based on market volatility and conditions", () => {
                ...
                const thresholdCalculator = detectorAny.adaptiveThresholdCalculator;
                ...
                expect(currentThresholds).toHaveProperty("consistencyRequirement");
                
                expect(currentThresholds.absorptionLevels).toHaveProperty("strong");
                expect(currentThresholds.absorptionLevels).toHaveProperty("moderate");
                expect(currentThresholds.absorptionLevels).toHaveProperty("weak");
                ...
                const hasSignificantChange = detectorAny.hasSignificantChange(
                    currentThresholds,
                    {
                        ...
                        absorptionLevels: {
                            ...
                            strong: currentThresholds.absorptionLevels.strong * 1.2 // 20% change
                        }
                    }
                );
                ...
                console.log("✅ Adaptive thresholds respond to market condition changes");
            });
        });
    });
    ...
    describe("7. Microstructure Integration", () => {
        describe("Algorithm and Execution Pattern Detection", () => {
            it("should analyze microstructure patterns for sophisticated execution detection", () => {
                console.log("🧪 Testing microstructure integration for execution patterns");
                ...
                expect(microstructureTrade.microstructure.suspectedAlgoType).toBe("TWAP");
                expect(microstructureTrade.microstructure.fragmentationScore).toBeGreaterThan(0.7);
                expect(microstructureTrade.microstructure.executionEfficiency).toBeGreaterThan(0.8);
                
                console.log("✅ Microstructure analysis detects sophisticated execution patterns");
            });
        });
    });
    ...
    describe("8. Error Handling and Edge Cases", () => {
        describe("Graceful Degradation", () => {
            it("should handle missing order book data gracefully", () => {
                console.log("🧪 Testing graceful handling of missing order book data");
    ...
    describe("9. Absorption Method Detection", () => {
        describe("Detection Method Classification", () => {
            it("should correctly classify absorption detection methods used", () => {
                ...
                const method = detectorAny.determineAbsorptionMethod(testZone, testPrice);
    ...
    describe("10. Comprehensive Behavior Validation", () => {
        describe("All Absorption Behaviors Integration", () => {
            it("should demonstrate comprehensive absorption behavior validation", () => {
                console.log("🧪 Running comprehensive absorption behavior validation");
                ...
                const absorptionCondition = detectorAny.checkAbsorptionConditions(testPrice, "buy", testZone);
                const zoneHistory = detectorAny.zonePassiveHistory.get(testZone);
                ...
                console.log("✅ All absorption behaviors validated successfully:");
