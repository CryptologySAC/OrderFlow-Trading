...
describe("AbsorptionDetector - Minimal Debug", () => {
    ...
    it("should debug exactly why no signals are generated", { timeout: 10000 }, async () => {
        ...
        console.log(`  Price: ${basePrice} (all trades same price = same zone)`);
        console.log(`  Zone calculation: price=${basePrice}, zoneTicks=3, precision=2`);
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    ...
                    console.log(`  ❌ FAILURE: No signal with ultra minimal settings`);
                    console.log(`  This confirms fundamental detection bug`);
                    console.log(`\n🔍 ANALYSIS:`);
                    console.log(`  - 5 trades at same price should be in same zone`);
                    console.log(`  - 100% selling pressure (500 sells, 0 buys)`);
                    console.log(`  - Ultra permissive thresholds (0.01 absorption, 1 minVol)`);
                    console.log(`  - All features disabled`);
                    console.log(`  - If this doesn't work, core detection logic is broken`);
                }
