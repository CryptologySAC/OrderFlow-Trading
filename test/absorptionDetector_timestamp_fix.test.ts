...
describe("AbsorptionDetector - Timestamp Fix", () => {
    ...
    it("should test if timestamp pruning is blocking signal generation", { timeout: 10000 }, async () => {
        ...
        detector.on("signalCandidate", (data) => {
            signalEmitted = true;
            console.log(`🎯 SIGNAL GENERATED: ${data.side} at ${data.price}`);
        });
        ...
        console.log(`  Using timestamps from last 30 seconds to ensure they're within window`);
        ...
        recentTrades.forEach((trade, i) => {
            ...
            console.log(`  Trade ${i + 1}: SELL ${trade.quantity} (${ageMs}ms ago, within ${ageMs < 60000 ? 'YES' : 'NO'} window)`);
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                if (signalEmitted) {
                    console.log(`  ✅ SUCCESS: Recent timestamps allow signal generation!`);
                    console.log(`  CONFIRMED: Timestamp pruning was the blocking issue`);
                } else {
                    console.log(`  ❌ STILL BLOCKED: Even recent timestamps don't work`);
                    console.log(`  The issue is deeper than timestamp pruning`);
                    
                    console.log(`\n🔍 DEBUGGING INFO:`);
                    console.log(`    All trades were within the 60-second window`);
                    console.log(`    Issue must be in zone aggregation or analysis logic`);
                }
