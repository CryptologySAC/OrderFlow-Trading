...
describe("Complete Signal Type Ecosystem Validation", () => {
    ...
    describe("Signal Type Contract Completeness", () => {
        it("should have signal type contracts for all 5 enhanced detectors", () => {
            ...
            console.log(
                "✅ All 5 enhanced detectors have signal type contracts"
            );
            console.log("📋 Enhanced Detector Signal Type Contracts:");
            Object.entries(ENHANCED_DETECTOR_CONTRACTS).forEach(
                ([detector, signalType]) => {
        ...
        it("should use only valid SignalType values", () => {
            ...
            console.log(
                "✅ All detector signal types are valid SignalType values"
            );
    ...
    describe("SignalManager Threshold Mapping", () => {
        it("should have threshold configuration for every detector signal type", () => {
            const detectorSignalTypes = Object.values(
                ENHANCED_DETECTOR_CONTRACTS
            );
            ...
            detectorSignalTypes.forEach((signalType) => {
                ...
                expect(
                    SIGNAL_MANAGER_THRESHOLDS[
                        signalType as keyof typeof SIGNAL_MANAGER_THRESHOLDS
                    ]
                ).toBeDefined();
                expect(
                    typeof SIGNAL_MANAGER_THRESHOLDS[
                        signalType as keyof typeof SIGNAL_MANAGER_THRESHOLDS
                    ]
                ).toBe("number");
            });
            ...
            console.log(
                "✅ All detector signal types have threshold configuration"
            );
            console.log("📊 Signal Type → Threshold Mapping:");
            Object.entries(SIGNAL_MANAGER_THRESHOLDS).forEach(
        ...
        it("should have realistic threshold values", () => {
            ...
            console.log(
                "✅ All thresholds are within realistic ranges (0.1 - 1.0)"
            );
        });
    ...
    describe("Frontend Statistics Compatibility", () => {
        it("should map all detector signal types to frontend statistics", () => {
            ...
            console.log(
                "✅ All detector signal types map to frontend statistics"
            );
            console.log("📈 Frontend Statistics Mapping:");
            Object.entries(FRONTEND_STATISTICS_TYPES).forEach(
                ([display, signalType]) => {
    ...
    describe("Signal Type Ecosystem Integrity", () => {
        it("should have complete signal processing pipeline coverage", () => {
            ...
            Object.entries(ENHANCED_DETECTOR_CONTRACTS).forEach(
                ([detector, signalType]) => {
                    ...
                    expect(SIGNAL_MANAGER_THRESHOLDS).toHaveProperty(
                        signalType
                    );

                    // Has frontend statistics mapping
                    const frontendTypes = Object.values(
                        FRONTEND_STATISTICS_TYPES
                    );
                    expect(frontendTypes).toContain(signalType);

                    console.log(
                        `✅ ${detector}: ${signalType} → threshold: ${SIGNAL_MANAGER_THRESHOLDS[signalType as keyof typeof SIGNAL_MANAGER_THRESHOLDS]} → frontend: ✓`
                    );
                }
            );
        });
        ...
        it("should prevent signal type mismatch issues", () => {
            ...
            if (unusedThresholds.length > 0) {
                console.log(
                    `⚠️  Unused threshold configurations: ${unusedThresholds.join(", ")}`
                );
            }

            console.log("✅ No signal type mismatches detected");
            console.log(
                "✅ All threshold configurations are used by detectors"
            );
        });
        ...
        it("should validate signal type consistency", () => {
            ...
            const hasUnderscores = signalTypes.filter((type) =>
                type.includes("_")
            );
            const hasNoUnderscores = signalTypes.filter(
            ...
            console.log(
                `   Without underscores: ${hasNoUnderscores.join(", ")}`
            );
        ...
        it("should document complete signal type ecosystem", () => {
            ...
            Object.entries(ENHANCED_DETECTOR_CONTRACTS).forEach(
                ([detector, signalType]) => {
                    ...
                    const frontendName = Object.keys(
                        FRONTEND_STATISTICS_TYPES
                    ).find(
                        (key) =>
                            FRONTEND_STATISTICS_TYPES[
                                key as keyof typeof FRONTEND_STATISTICS_TYPES
                            ] === signalType
                    );
            ...
            console.log(
                "   This test suite prevents signal type mismatch issues like:"
            );
            console.log(
                "   • Detectors emitting 'accumulation' but config expecting 'accumulation_zone'"
            );
            console.log(
                "   • Missing threshold configurations causing fallback to global threshold"
            );
            console.log(
                "   • Signals being generated but not appearing in frontend statistics"
            );
            console.log(
                "   • Type casting violations (as SignalType) in detector implementations"
            );
