...
export class PropertyTestDataGenerator {
     ...
     */
    generateNormalTrades(count: number, basePrice: number = 100): EnrichedTradeEvent[] {
     ...
     */
    generateVolatileTrades(count: number, basePrice: number = 100): EnrichedTradeEvent[] {
        const trades: EnrichedTradeEvent[] = [];
        let currentPrice = basePrice;
     ...
     */
    generateSparseTrades(count: number, basePrice: number = 100): EnrichedTradeEvent[] {
        const trades: EnrichedTradeEvent[] = [];
        let currentPrice = basePrice;
...
export class MathematicalPropertyValidator {
    ...
    static validateNumericalStability(value: unknown, context: string): void {
        if (typeof value === 'number') {
            expect(isFinite(value), `${context}: Value should be finite, got ${value}`).toBe(true);
            expect(!isNaN(value), `${context}: Value should not be NaN`).toBe(true);
            expect(value !== Infinity, `${context}: Value should not be Infinity`).toBe(true);
            expect(value !== -Infinity, `${context}: Value should not be -Infinity`).toBe(true);
    ...
    static validateConfidenceBounds(confidence: number, context: string): void {
        ...
        expect(confidence, `${context}: Confidence should be >= 0`).toBeGreaterThanOrEqual(0);
        expect(confidence, `${context}: Confidence should be <= 1`).toBeLessThanOrEqual(1);
    }
    ...
    static validatePriceBounds(price: number, context: string): void {
        ...
        expect(price, `${context}: Price should be positive`).toBeGreaterThan(0);
        expect(price, `${context}: Price should be reasonable (<= 1M)`).toBeLessThanOrEqual(1000000);
    }
    ...
    static validateQuantityBounds(quantity: number, context: string): void {
        ...
        expect(quantity, `${context}: Quantity should be positive`).toBeGreaterThan(0);
        expect(quantity, `${context}: Quantity should be reasonable (<= 1M)`).toBeLessThanOrEqual(1000000);
    }
    ...
    static validateMonotonicity<T>(
        inputs: T[],
        outputs: number[],
        direction: 'increasing' | 'decreasing' | 'non-decreasing' | 'non-increasing',
        context: string
    ): void {
        expect(inputs.length, `${context}: Inputs and outputs should have same length`).toBe(outputs.length);
        
        for (let i = 1; i < outputs.length; i++) {
            const prev = outputs[i - 1];
            const curr = outputs[i];
            
            this.validateNumericalStability(prev, `${context}.output[${i-1}]`);
            this.validateNumericalStability(curr, `${context}.output[${i}]`);
            
            switch (direction) {
                case 'increasing':
                    expect(curr, `${context}: Output should be strictly increasing at index ${i}`).toBeGreaterThan(prev);
                    break;
                case 'decreasing':
                    expect(curr, `${context}: Output should be strictly decreasing at index ${i}`).toBeLessThan(prev);
                    break;
                case 'non-decreasing':
                    expect(curr, `${context}: Output should be non-decreasing at index ${i}`).toBeGreaterThanOrEqual(prev);
                    break;
                case 'non-increasing':
                    expect(curr, `${context}: Output should be non-increasing at index ${i}`).toBeLessThanOrEqual(prev);
                    break;
            }
        }
    }
    ...
    static validateConsistency<T>(
        ...
        for (let i = 1; i < results.length; i++) {
            ...
            expect(diff, `${context}: Result ${i} should be consistent with baseline (diff: ${diff}, tolerance: ${tolerance})`).toBeLessThanOrEqual(tolerance);
        }
    }
     ...
     */
    static validateSignalProperties(signal: SignalCandidate, context: string): void {
        ...
        expect(signal.type, `${context}: Signal should have type`).toBeDefined();
        expect(signal.side, `${context}: Signal should have side`).toMatch(/^(buy|sell)$/);
        
        // Numerical validation
        this.validateConfidenceBounds(signal.confidence, context);
        this.validateNumericalStability(signal.timestamp, `${context}.timestamp`);
        expect(signal.timestamp, `${context}: Timestamp should be positive`).toBeGreaterThan(0);
        ...
        if (signal.data) {
            ...
            if (typeof signal.data === 'object' && 'price' in signal.data) {
                this.validatePriceBounds(signal.data.price as number, `${context}.data`);
            }
        }
    }
    ...
    static validateDetectorState(detector: any, context: string): void {
        ...
        expect(detector, `${context}: Detector should be defined`).toBeDefined();
        
        // Check if detector has getStatus method and validate its output
        if (typeof detector.getStatus === 'function') {
            const status = detector.getStatus();
            expect(typeof status, `${context}: Status should be string`).toBe('string');
            expect(status.length, `${context}: Status should not be empty`).toBeGreaterThan(0);
        }
    ...
    static validateConvergence(
        ...
        expect(values.length, `${context}: Should have enough samples for convergence test`).toBeGreaterThanOrEqual(minSamples);
        ...
        for (let i = 0; i < recentValues.length; i++) {
            ...
            this.validateNumericalStability(value, `${context}.convergence[${i}]`);
            expect(value, `${context}: Converged value ${i} should be >= ${expectedMin}`).toBeGreaterThanOrEqual(expectedMin);
            expect(value, `${context}: Converged value ${i} should be <= ${expectedMax}`).toBeLessThanOrEqual(expectedMax);
        }
    }
}
...
export class PropertyTestRunner {
    ...
    constructor(config: Partial<PropertyTestConfig> = {}) {
        ...
        this.dataGenerator = new PropertyTestDataGenerator(this.config.randomSeed);
    }
    ...
    async runDetectorPropertyTests<T>(
        ...
        await this.testNumericalStability(detectorFactory, detectorProcessor, signalCollector, testName);
        
        // Test 2: Edge Case Handling
        await this.testEdgeCaseHandling(detectorFactory, detectorProcessor, signalCollector, testName);
        
        // Test 3: Consistency under Repeated Input
        await this.testConsistency(detectorFactory, detectorProcessor, signalCollector, testName);
        
        // Test 4: Behavior under Market Volatility
        await this.testVolatilityHandling(detectorFactory, detectorProcessor, signalCollector, testName);
        
        // Test 5: Signal Quality Properties
        await this.testSignalQuality(detectorFactory, detectorProcessor, signalCollector, testName);
    ...
    private async testNumericalStability<T>(
        ...
        for (const trade of trades) {
            ...
            MathematicalPropertyValidator.validateDetectorState(detector, `${testName}.normalConditions`);
        }
        
        // Validate all signals produced
        const signals = signalCollector(detector);
        signals.forEach((signal, i) => {
            MathematicalPropertyValidator.validateSignalProperties(signal, `${testName}.normalConditions.signal[${i}]`);
        });
    }
    ...
    private async testEdgeCaseHandling<T>(
        ...
        for (const trade of extremeTrades) {
            ...
            MathematicalPropertyValidator.validateDetectorState(detector, `${testName}.extremeValues`);
        }
        
        // Validate signals remain valid even with extreme inputs
        const signals = signalCollector(detector);
        signals.forEach((signal, i) => {
            MathematicalPropertyValidator.validateSignalProperties(signal, `${testName}.extremeValues.signal[${i}]`);
        });
    }
    ...
    private async testVolatilityHandling<T>(
        ...
        for (const trade of volatileTrades) {
            ...
            MathematicalPropertyValidator.validateDetectorState(detector, `${testName}.volatility`);
        }
        
        // Validate signals during volatility
        const signals = signalCollector(detector);
        signals.forEach((signal, i) => {
            MathematicalPropertyValidator.validateSignalProperties(signal, `${testName}.volatility.signal[${i}]`);
        });
    }
    ...
    private async testSignalQuality<T>(
        ...
        const detector = detectorFactory();
        const trendingTrades = this.dataGenerator.generateTrendingTrades(500, 100, 'up');
