...
export class PerformanceBenchmark extends EventEmitter {
    ...
    public async stressTest(
        ...
        for (const multiplier of loadMultipliers) {
            ...
            const stressTrades = this.createStressTestData(
                baseTrades,
                multiplier
            );
            ...
            this.logger.info(`Running stress test - ${multiplier}x load`, {
                ...
                estimatedTps:
                    stressTrades.length /
                    (this.benchmarkConfig.maxDuration / 1000),
            });

            const metrics = await this.benchmarkDetector(
                detector,
                stressTrades
            );
            ...
            if (multiplier > 1) {
                ...
                const degradationRatio =
                    metrics.latency.average / baselineMetrics.latency.average;

                if (degradationRatio > 5.0) {
                    this.logger.warn(
                        "Performance degradation detected, stopping stress test",
                        {
                            multiplier,
                            degradationRatio,
    ...
    private async initializeBenchmark(): Promise<void> {
        ...
        if (
            this.benchmarkConfig.collectGCStats &&
            (process as any).getHeapStatistics
        ) {
        ...
        for (let i = 0; i < trades.length && this.isRunning; i++) {
            ...
            try {
                if (detector.instance.onEnrichedTrade) {
                    ...
                    throw new Error(
                        `Detector ${detector.detectorName} has no processable method`
                    );
                }
            } catch (error) {
            ...
            if (this.tradeCount >= this.benchmarkConfig.warmupTrades) {
                if (!this.warmupComplete) {
                    ...
                    this.logger.debug(
                        "Warmup complete, starting metric collection"
                    );
                }
    ...
    private calculateMetrics(): PerformanceMetrics {
        ...
        const throughputMetrics = {
            ...
            eventsPerSecond:
                (effectiveTrades + this.signalSamples.length) /
                (duration / 1000),
    ...
    private calculateSignalQuality(): PerformanceMetrics["signalQuality"] {
        ...
        return {
            ...
            averageConfidence:
                validSignals > 0 ? confidenceSum / validSignals : 0,
            validSignals,
            invalidSignals,
        };
    }
...
export class BenchmarkValidator {
    ...
    public static validateMetrics(
        ...
        if (metrics.latency.p99 > thresholds.maxLatencyMs) {
            violations.push(
                `P99 latency ${metrics.latency.p99.toFixed(2)}ms exceeds threshold ${thresholds.maxLatencyMs}ms`
            );
        }
        ...
        if (metrics.throughput.tradesPerSecond < thresholds.minThroughputTps) {
            violations.push(
                `Throughput ${metrics.throughput.tradesPerSecond.toFixed(2)} TPS below threshold ${thresholds.minThroughputTps} TPS`
            );
        }
        ...
        if (memoryGrowthMB > thresholds.maxMemoryGrowthMB) {
            violations.push(
                `Memory growth ${memoryGrowthMB.toFixed(2)}MB exceeds threshold ${thresholds.maxMemoryGrowthMB}MB`
            );
        }

        if (
            metrics.memory.growthPercentage > thresholds.maxMemoryGrowthPercent
        ) {
            violations.push(
                `Memory growth ${metrics.memory.growthPercentage.toFixed(2)}% exceeds threshold ${thresholds.maxMemoryGrowthPercent}%`
            );
        }

        // Signal quality validation
        if (
            metrics.signalQuality.averageConfidence <
            thresholds.minSignalConfidence
        ) {
            violations.push(
                `Average signal confidence ${metrics.signalQuality.averageConfidence.toFixed(3)} below threshold ${thresholds.minSignalConfidence}`
            );
        }
