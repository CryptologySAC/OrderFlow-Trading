...
describe('Signal Analysis Mathematical Validation', () => {
    describe('Basic Mathematical Invariants', () => {
        it('should validate that individual signal movements cannot exceed total market movement', () => {
            ...
            const totalMarketMovement = ((marketHigh - marketLow) / marketHigh) * 100;
            
            // Then NO signal can achieve more than this movement
            const signalEntry = 131.26;
            const signalExit = 129.59;
            const signalMovement = ((signalEntry - signalExit) / signalEntry) * 100;
            ...
            // Verify the math
            expect(totalMarketMovement).toBeCloseTo(1.370, 2);
        ...
        it('should validate that signals entering at lower prices achieve less percentage gain', () => {
            ...
            const highEntryGain = ((highEntry - commonExitPrice) / highEntry) * 100;
            
            // Lower entry should yield lower percentage
            const lowEntry = 131.19;
            const lowEntryGain = ((lowEntry - commonExitPrice) / lowEntry) * 100;
            
            expect(highEntryGain).toBeGreaterThan(lowEntryGain);
            expect(highEntryGain).toBeCloseTo(1.370, 2);
            expect(lowEntryGain).toBeCloseTo(1.220, 2);
        });
        ...
        it('should validate phase movement calculation consistency', () => {
            ...
            const wrongPhaseMovement = ((wrongPhaseStart - finalLow) / wrongPhaseStart) * 100;
            
            // CORRECT: Using first chronological signal for phase start
            const correctPhaseStart = signals[0].price;
            const correctPhaseMovement = ((correctPhaseStart - finalLow) / correctPhaseStart) * 100;
            ...
            const firstSignalMovement = ((signals[0].price - finalLow) / signals[0].price) * 100;
    ...
    describe('Phase Detection Logic Validation', () => {
        it('should correctly identify separate phases based on time gaps', () => {
            const phase1EndTime = new Date('2025-08-12T19:18:00-05:00').getTime();
            const phase2StartTime = new Date('2025-08-12T19:48:14-05:00').getTime();
            
            const timeGapMinutes = (phase2StartTime - phase1EndTime) / (60 * 1000);
        ...
        it('should correctly identify separate phases based on price retracement', () => {
            ...
            const retracement = ((phase2EntryPrice - phase1TPLevel) / phase1TPLevel) * 100;
    ...
    describe('Target Achievement Validation', () => {
        it('should correctly identify when 0.7% target is reached', () => {
            ...
            const successMovement = (successEntry - successActual) / successEntry;
        ...
        it('should show actual maximum movement, not capped at 0.7%', () => {
            const signals = [
                ...
                { entry: 131.39, maxReached: 129.59, expectedTP: 1.370 },
                { entry: 130.89, maxReached: 129.59, expectedTP: 0.993 }
            ];
            
            for (const signal of signals) {
                const actualMovement = ((signal.entry - signal.maxReached) / signal.entry) * 100;
    ...
    describe('Cluster Detection Validation', () => {
        it('should group signals within time and price proximity', () => {
            ...
            const cluster1Signals = [
                { time: new Date('2025-08-12T19:15:05-05:00').getTime(), price: 131.26 },
                { time: new Date('2025-08-12T19:15:52-05:00').getTime(), price: 131.29 },
                { time: new Date('2025-08-12T19:18:00-05:00').getTime(), price: 131.26 }
            ];
            ...
            const priceRange = (Math.max(...prices) - Math.min(...prices)) / Math.max(...prices);
    ...
    describe('Data Consistency Validation', () => {
        ...
        it('should never show contradictory phase vs signal movements', () => {
            ...
            const signalMovements = [1.272, 1.295, 1.280, 1.370];
            
            // This is INVALID - signals cannot exceed phase movement
            const isContradictory = signalMovements.some(s => s > phaseMovement);
            ...
            const allSignalsValid = signalMovements.every(s => s <= correctPhaseMovement);
    ...
    describe('Success Rate Calculation', () => {
        it('should correctly calculate success rates', () => {
            ...
            const totalSignals = phase1.total + phase2.total + phase3.total + phase4.total;
            const totalSuccessful = phase1.successful + phase2.successful + phase3.successful + phase4.successful;
...
describe('Required Fixes for Correct Implementation', () => {
    it('should use FinancialMath for all calculations', () => {
        ...
        const calculatePercentageChange = (start: number, end: number): number => {
    ...
    it('should calculate phase movement from actual market highs/lows during signal period', () => {
        ...
        const correctPhaseMovement = ((marketHighAtSignals - marketLowDuringPhase) / marketHighAtSignals) * 100;
    ...
    it('should return null when calculations are invalid', () => {
        const calculateMovement = (entry: number, exit: number): number | null => {
