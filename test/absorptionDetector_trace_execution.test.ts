...
describe("AbsorptionDetector - Execution Trace", () => {
    ...
    it(
        "should trace execution flow to find blocking point",
        { timeout: 10000 },
        async () => {
            console.log(
            ...
            console.log(
                `📊 Creating AbsorptionDetector with trace settings...`
            );
            ...
            console.log(
                `  Logger.info called: ${(mockLogger.info as any).mock.calls.length} times`
            );
            console.log(
                `  Logger.warn called: ${(mockLogger.warn as any).mock.calls.length} times`
            );
            console.log(
                `  Logger.error called: ${(mockLogger.error as any).mock.calls.length} times`
            );
            ...
            return new Promise<void>((resolve) => {
                setTimeout(() => {
                    ...
                    if (!signalEmitted) {
                        console.log(
                            `  ❌ No signal generated despite 5 trades and ultra permissive settings`
                        );
                        console.log(`\n📋 Logger Call Summary:`);
                        console.log(
                            `    Info calls: ${(mockLogger.info as any).mock.calls.length}`
                        );
                        console.log(
                            `    Warn calls: ${(mockLogger.warn as any).mock.calls.length}`
                        );
                        console.log(
                            `    Error calls: ${(mockLogger.error as any).mock.calls.length}`
                        );

                        if ((mockLogger.info as any).mock.calls.length === 0) {
                            console.log(
                                `  🚨 CRITICAL: No log messages at all - detector methods not being called`
                            );
                        }
                    }
