...
const createDetailedDebugLogger = (): ILogger => ({
    info: vi.fn((msg, data) => {
        console.log(`[DETAILED-INFO] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    warn: vi.fn((msg, data) => {
        console.log(`[DETAILED-WARN] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    error: vi.fn((msg, data) => {
        console.log(`[DETAILED-ERROR] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
    debug: vi.fn((msg, data) => {
        console.log(`[DETAILED-DEBUG] ${msg}`, data ? JSON.stringify(data, null, 2) : '');
    }),
});
...
describe("AbsorptionDetector - Detailed Debug", () => {
    ...
    it("should trace null returns in signal generation", { timeout: 5000 }, () => {
        ...
        // Spy on key methods
        const getAbsorbingSideForZoneSpy = vi.spyOn(detector as any, 'getAbsorbingSideForZone');
        const getDominantAggressiveSideSpy = vi.spyOn(detector as any, 'getDominantAggressiveSide');
        const calculatePriceEfficiencySpy = vi.spyOn(detector as any, 'calculatePriceEfficiency');
        const checkAbsorptionConditionsSpy = vi.spyOn(detector as any, 'checkAbsorptionConditions');
        ...
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                console.log(`   Called: ${getAbsorbingSideForZoneSpy.mock.calls.length} times`);
                if (getAbsorbingSideForZoneSpy.mock.calls.length > 0) {
                    const lastResult = getAbsorbingSideForZoneSpy.mock.results[getAbsorbingSideForZoneSpy.mock.results.length - 1];
                    console.log(`   Last return: ${lastResult.value}`);
                    if (lastResult.value === null) {
                        console.log(`   ❌ Returning null - blocking signal generation!`);
                    }
                }
                
                // getDominantAggressiveSide analysis
                console.log(`\n2️⃣ getDominantAggressiveSide:`);
                console.log(`   Called: ${getDominantAggressiveSideSpy.mock.calls.length} times`);
                if (getDominantAggressiveSideSpy.mock.calls.length > 0) {
                    const lastResult = getDominantAggressiveSideSpy.mock.results[getDominantAggressiveSideSpy.mock.results.length - 1];
                    console.log(`   Last return: ${lastResult.value}`);
                    if (lastResult.value === null) {
                        console.log(`   ❌ Returning null - insufficient data for dominant side!`);
                        const lastCall = getDominantAggressiveSideSpy.mock.calls[getDominantAggressiveSideSpy.mock.calls.length - 1];
                        console.log(`   Trades passed: ${lastCall[0].length}`);
                    }
                }
                
                // calculatePriceEfficiency analysis
                console.log(`\n3️⃣ calculatePriceEfficiency:`);
                console.log(`   Called: ${calculatePriceEfficiencySpy.mock.calls.length} times`);
                if (calculatePriceEfficiencySpy.mock.calls.length > 0) {
                    const lastResult = calculatePriceEfficiencySpy.mock.results[calculatePriceEfficiencySpy.mock.results.length - 1];
                    console.log(`   Last return: ${lastResult.value}`);
                    if (lastResult.value === null) {
                        console.log(`   ❌ Returning null - price efficiency calculation failed!`);
                    }
                }
                
                // checkAbsorptionConditions analysis
                console.log(`\n4️⃣ checkAbsorptionConditions:`);
                console.log(`   Called: ${checkAbsorptionConditionsSpy.mock.calls.length} times`);
                if (checkAbsorptionConditionsSpy.mock.calls.length > 0) {
                    const lastResult = checkAbsorptionConditionsSpy.mock.results[checkAbsorptionConditionsSpy.mock.results.length - 1];
                    console.log(`   Last return:`, lastResult.value);
                }
                ...
                console.log(`\n🔧 DIAGNOSIS:`);
                if (!signalEmitted) {
                    if (getAbsorbingSideForZoneSpy.mock.results.some(r => r.value === null)) {
                        console.log(`   Primary issue: getAbsorbingSideForZone returning null`);
                        if (getDominantAggressiveSideSpy.mock.results.some(r => r.value === null)) {
                            console.log(`   Root cause: getDominantAggressiveSide returning null`);
                            console.log(`   Fix needed: Check trade count and time window logic`);
                        } else if (calculatePriceEfficiencySpy.mock.results.some(r => r.value === null)) {
                            console.log(`   Root cause: calculatePriceEfficiency returning null`);
                            console.log(`   Fix needed: Check price movement calculation`);
                        }
                    } else if (checkAbsorptionConditionsSpy.mock.calls.length === 0) {
                        console.log(`   Issue: checkAbsorptionConditions never called`);
                        console.log(`   Fix needed: Check early returns in analyzeZoneForAbsorption`);
                    }
                }
