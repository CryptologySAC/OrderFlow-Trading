...
describe("AbsorptionDetector - Business Logic Validation", () => {
    ...
    describe("Price Efficiency Analysis (Core Business Logic)", () => {
        it("should detect institutional absorption when price efficiency is below threshold", () => {
            console.log("🎯 TESTING: Price efficiency analysis - core absorption detection");
            ...
            const totalVolume = highVolumeMinimalMovement.reduce((sum, t) => sum + t.quantity, 0);
            const priceRange = Math.max(...highVolumeMinimalMovement.map(t => t.price)) - 
                             Math.min(...highVolumeMinimalMovement.map(t => t.price));
            
            console.log(`🎯 Business scenario: ${totalVolume} volume, ${priceRange.toFixed(4)} price range`);
            ...
            console.log(`🎯 Price efficiency: ${actualEfficiency.toFixed(3)} (threshold: 0.85)`);
        ...
        it("should NOT detect absorption when price efficiency is above threshold", () => {
            ...
            const totalVolume = normalEfficiencyTrades.reduce((sum, t) => sum + t.quantity, 0);
            const priceRange = Math.max(...normalEfficiencyTrades.map(t => t.price)) - 
            ...
            console.log(`🎯 Normal efficiency: ${actualEfficiency.toFixed(3)} (should be > 0.85)`);
        ...
        it("should detect volume-price divergence indicating hidden institutional orders", () => {
            ...
            const institutionalVolume = institutionalTrades.reduce((sum, t) => sum + t.quantity, 0);
            const institutionalRange = Math.max(...institutionalTrades.map(t => t.price)) - 
                                    Math.min(...institutionalTrades.map(t => t.price));
            
            console.log(`🎯 Institutional scenario: ${institutionalVolume} volume, ${institutionalRange.toFixed(4)} range`);
    ...
    describe("Volume Surge Integration with Price Efficiency", () => {
        it("should enhance absorption confidence when volume surge accompanies price inefficiency", () => {
            console.log("🎯 TESTING: Volume surge enhancement of absorption detection");
            ...
            detector.on("signalCandidate", (signal) => {
                ...
                console.log(`🎯 Enhanced signal confidence: ${signal.confidence}`);
            });
            ...
            const totalVolume = surgeAbsorption.reduce((sum, t) => sum + t.quantity, 0);
            const surgeVolume = surgeAbsorption.slice(4).reduce((sum, t) => sum + t.quantity, 0);
            const baselineVolume = surgeAbsorption.slice(0, 4).reduce((sum, t) => sum + t.quantity, 0);
            
            const volumeMultiplier = (surgeVolume / 8) / (baselineVolume / 4);
            
            console.log(`🎯 Volume analysis: baseline=${baselineVolume}, surge=${surgeVolume}, multiplier=${volumeMultiplier.toFixed(2)}`);
    ...
    describe("Configuration Validation", () => {
        it("should respect priceEfficiencyThreshold configuration parameter", () => {
            ...
            const volume = moderateInefficiency.reduce((sum, t) => sum + t.quantity, 0);
            const range = Math.max(...moderateInefficiency.map(t => t.price)) - 
                         Math.min(...moderateInefficiency.map(t => t.price));
            
            const efficiency = range / ((volume / 120) * TICK_SIZE * 10);
            
            console.log(`🎯 Efficiency with strict threshold: ${efficiency.toFixed(3)} (threshold: 0.95)`);
    ...
    describe("Real Market Scenarios", () => {
        it("should detect institutional support level absorption", () => {
            console.log("🎯 TESTING: Institutional support level absorption scenario");
            ...
            const supportVolume = supportTrades.reduce((sum, t) => sum + t.quantity, 0);
            const supportRange = Math.max(...supportTrades.map(t => t.price)) - 
                                Math.min(...supportTrades.map(t => t.price));
            
            console.log(`🎯 Support defense: ${supportVolume} volume, ${supportRange.toFixed(1)} range at ${supportLevel}`);
