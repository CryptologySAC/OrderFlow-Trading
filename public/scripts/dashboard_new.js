...
function updateOrderBookChart(chart) {
    if (!chart) return;
    const labels = orderBookData.priceLevels.map((level) =>
        level.price.toFixed(2)
    );
    const bids = orderBookData.priceLevels.map((level) => level.bid || 0);
    const asks = orderBookData.priceLevels.map((level) => level.ask || 0);
    const bidColors = bids.map((bid) =>
        bid ? `rgba(0, 128, 0, ${Math.min(bid / 2000, 1)})` : "rgba(0, 0, 0, 0)"
    );
    const askColors = asks.map((ask) =>
        ask ? `rgba(255, 0, 0, ${Math.min(ask / 2000, 1)})` : "rgba(0, 0, 0, 0)"
    );
...
function updateIndicators() {
    ...
    setText(
        "directionText",
        `Direction: ${orderBookData.direction.type} (${orderBookData.direction.probability}%)`,
    ...
    setText(
        "ratioText",
        `Ask/Bid Ratio: ${orderBookData.ratio.toFixed(2)} (Threshold: 2)`
    );
    setText(
        "supportText",
        `Bid Support: ${orderBookData.supportPercent.toFixed(2)}% (Threshold: 50%)`
    );
    setText(
        "stabilityText",
        `Ask Volume Stability: ${orderBookData.askStable ? "Stable" : "Unstable"}`,
        orderBookData.askStable ? "green" : "red"
    );
    setText(
        "volumeImbalance",
        `Volume Imbalance: ${orderBookData.volumeImbalance.toFixed(2)} (Short < -0.65 | Long > 0.65)`,
