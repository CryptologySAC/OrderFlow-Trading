...
class Storage {
    ...
    getAggregatedTrades;
    constructor() {
        this.db = new better_sqlite3_1.default("trades.db", { verbose: console.log });
    ...
    saveTrade(trade) {
        try {
            ...
            this.updateFeedState.run({
                ...
                lastAggregatedTradeId: this.getLastFeedState().lastAggregatedTradeId,
                lastAggregatedTradeTime: this.getLastFeedState().lastAggregatedTradeTime,
                updatedAt: Date.now(),
            });
        }
        catch (error) {
     ...
     */
    saveAggregatedTrade(aggTrade, symbol) {
        try {
            const orderType = aggTrade.m ? "SELL" : "BUY";
            this.insertAggregatedTrade.run({
                aggregatedTradeId: aggTrade.a,
                firstTradeId: aggTrade.f !== undefined ? Math.trunc(aggTrade.f) : 0,
                lastTradeId: aggTrade.l !== undefined ? Math.trunc(aggTrade.l) : 0,
                tradeTime: aggTrade.T !== undefined ? Math.trunc(aggTrade.T) : 0,
                symbol,
    ...
    getLastFeedState() {
        const result = this.getFeedState.get();
        return {
     ...
     */
    getLatestTrades(n, symbol = "LTCUSDT") {
        try {
            const rows = this.getTrades.all({ symbol, limit: n });
            return rows;
        }
        catch (error) {
     ...
     */
    getLatestAggregatedTrades(n, symbol = "LTCUSDT") {
        try {
            const rows = this.getAggregatedTrades.all({ symbol, limit: n });
            return rows.map((row) => ({
                e: "aggTrade",
                s: row.symbol,
