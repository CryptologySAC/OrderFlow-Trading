...
interface WorkerInfo {
    loggerWorker?: string;
    binanceWorker?: string;
    communicationWorker?: string;
...
function updateTables(metrics: MetricsData): void {
    ...
    if (histBody) {
        ...
        for (const [name, summary] of histogramEntries) {
            ...
            const row = document.createElement("tr");
            const displayName = formatMetricName(name);
            const count = formatNumber((summary as HistogramData).count || 0);
            ...
            if (
                ...
                minDisplay = (summary as HistogramData).min
                    ? `${(summary as HistogramData).min!.toFixed(2)} ms`
                    : "0 ms";
...
function formatMetricValue(name: string, value: any): string {
...
function updateSignalMetrics(
    ...
    const getCounterValue = (name: string): number => {
        const counter = counters[name];
        return typeof counter === "object" ? counter.value || 0 : counter || 0;
    };

    // Helper function to get rate per minute
    const getRate = (count: number, timeFrameMinutes: number = 1): string => {
    ...
    updateElement(
        "avgConfidence",
        confidenceHist
            ? ((confidenceHist as HistogramData).mean || 0).toFixed(3)
            : "--"
    );
    updateElement(
        "confidenceP50",
        confidenceHist
            ? getPercentile(confidenceHist as HistogramData, 50).toFixed(3)
            : "--"
    );
    updateElement(
        "confidenceP95",
        confidenceHist
            ? getPercentile(confidenceHist as HistogramData, 95).toFixed(3)
            : "--"
    );
    updateElement(
        "avgCorrelationStrength",
        correlationHist
            ? ((correlationHist as HistogramData).mean || 0).toFixed(3)
            : "--"
    );
    updateElement(
        "avgProcessingTime",
