// Persistence utilities for the trading dashboard
// Handles localStorage operations for saving and restoring dashboard settings
...
import { triggerChartResize } from "./ui.js";
...
interface ColumnWidthSettings {
    column1: number;
    column3: number;
    timestamp: number;
}

interface AnomalyFilterSettings {
    filters: string[];
    timestamp: number;
}
...
export function saveColumnWidths(): void {
    try {
        const dashboard: Element | null = document.querySelector(".dashboard");
        const column1: HTMLElement | null = document.getElementById("column1");
        const column3: HTMLElement | null = document.getElementById("column3");

        if (!dashboard || !column1 || !column3) {
            console.warn(
                "Required dashboard elements not found for saving column widths"
            );
            return;
        }

        const dashboardRect: DOMRect = dashboard.getBoundingClientRect();
        const column1Rect: DOMRect = column1.getBoundingClientRect();
        const column3Rect: DOMRect = column3.getBoundingClientRect();

        const column1Percent: number =
            (column1Rect.width / dashboardRect.width) * 100;
        const column3Percent: number =
            (column3Rect.width / dashboardRect.width) * 100;

        const columnWidths: ColumnWidthSettings = {
...
export function saveAnomalyFilters(): void {
    try {
        const filterSettings: AnomalyFilterSettings = {
...
export function restoreAnomalyFilters(): void {
    try {
        const savedFilters: string | null = localStorage.getItem(
            "dashboardAnomalyFilters"
        );
        ...
        const filterSettings: AnomalyFilterSettings = JSON.parse(savedFilters);
        ...
        const filterBox: Element | null =
            document.querySelector(".anomaly-filter");
        if (filterBox) {
            filterBox
...
export function saveTimeRange(): void {
    try {
        const rangeSettings: TimeRangeSettings = {
...
export function restoreTimeRange(): void {
    try {
        const savedRange: string | null =
            localStorage.getItem("dashboardTimeRange");
        ...
        const rangeSettings: TimeRangeSettings = JSON.parse(savedRange);
        ...
        const rangeSelector: Element | null =
            document.querySelector(".rangeSelector");
        if (rangeSelector) {
            rangeSelector
                .querySelectorAll("button")
                .forEach((button: HTMLButtonElement) => {
                    const buttonRange: string | null =
                        button.getAttribute("data-range");
                    const buttonRangeValue: number | null =
                        buttonRange === "all"
                            ? null
                            : buttonRange
        ...
        // Note: This will be handled by the chart initialization
    } catch (error: unknown) {
...
export function saveVerticalLayout(): void {
    try {
        const layoutSettings: VerticalLayoutSettings = {
...
export function restoreVerticalLayout(): void {
    try {
        const savedLayout: string | null = localStorage.getItem(
            "dashboardVerticalLayout"
        );
        ...
        const layoutSettings: VerticalLayoutSettings = JSON.parse(savedLayout);
...
export function restoreColumnWidths(): void {
    try {
        const savedWidths: string | null = localStorage.getItem(
            "dashboardColumnWidths"
        );
        ...
        const columnWidths: ColumnWidthSettings = JSON.parse(savedWidths);
        ...
        if (column1 && column3) {
            ...
            const column1Width: number = Math.min(
                Math.max(columnWidths.column1, 15),
                50
            ); // 15-50%
            const column3Width: number = Math.min(
