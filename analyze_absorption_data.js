...
class AbsorptionDataAnalyzer {
        ...
        for (const line of lines) {
            if (
                ...
                try {
                    ...
                    if (
                        jsonData.passiveVolumeRatio &&
                        jsonData.passiveVolume &&
                        ...
                        this.volumeData.push({
                            ...
                            totalVolume: parseFloat(jsonData.totalVolume),
                            aggressiveVolume:
                                parseFloat(jsonData.totalVolume) -
                                parseFloat(jsonData.passiveVolume),
                        });
                    }
                } catch (e) {
                    // Skip malformed JSON
            ...
            if (
                line.includes("Passive volume ratio below threshold - REJECTED")
            ) {
    ...
    calculateRatioStatistics() {
        ...
        const ratios = this.passiveVolumeRatios
            .map((d) => d.ratio)
            .sort((a, b) => a - b);
    ...
    calculateVolumeAnalysis() {
        ...
        const aggressiveVolumes = this.volumeData.map(
            (d) => d.aggressiveVolume
        );
        ...
        return {
            passiveVolume: {
                mean:
                    passiveVolumes.reduce((a, b) => a + b, 0) /
                    passiveVolumes.length,
                median: this.calculatePercentile(
                    passiveVolumes.sort((a, b) => a - b),
                    50
                ),
            ...
            aggressiveVolume: {
                mean:
                    aggressiveVolumes.reduce((a, b) => a + b, 0) /
                    aggressiveVolumes.length,
                median: this.calculatePercentile(
                    aggressiveVolumes.sort((a, b) => a - b),
                    50
                ),
    ...
    generateRecommendations(ratioStats) {
        const recommendations = {
            current: {
                ...
                signalsBlocked: "100%",
                reasoning:
                    "Current 99.7% threshold blocks ALL legitimate absorption patterns",
            },
            conservative: {
            ...
            recommended: {
                ...
                reasoning:
                    "Balanced approach: 90th percentile passive ratio + realistic confidence and price efficiency thresholds",
            },
        };
                    ...
                    changes: {
                        passiveAbsorptionThreshold:
                            recommendations.recommended
                                .passiveAbsorptionThreshold,
                        finalConfidenceRequired:
                            recommendations.recommended.finalConfidenceRequired,
                        priceEfficiencyThreshold:
                            recommendations.recommended
                                .priceEfficiencyThreshold,
                    },
                    expectedImpact:
                        "Restore absorption signal generation capability",
                    riskLevel:
                        "LOW - Moving from impossible to realistic thresholds",
                },
                phase2: {
                    description: "A/B testing and optimization",
                    strategy:
                        "Test conservative vs balanced vs aggressive configurations",
                    metrics:
                        "Signal quality, false positive rate, turning point detection accuracy",
                    duration: "2-4 weeks",
                },
            },
        };
...
async function main() {
    ...
    try {
        ...
        console.log(
            "🧮 Calculating statistics and generating recommendations..."
        );
        ...
        const reportPath = path.join(
            __dirname,
            "absorption_optimization_report.json"
        );
        ...
        if (report.analysis.ratioStatistics) {
            ...
            console.log(
                `   Mean: ${report.analysis.ratioStatistics.mean.toFixed(6)}`
            );
            console.log(
                `   Median: ${report.analysis.ratioStatistics.median.toFixed(6)}`
            );
            console.log(
                `   95th percentile: ${report.analysis.ratioStatistics.p95.toFixed(6)}`
            );
            console.log(
                `   90th percentile: ${report.analysis.ratioStatistics.p90.toFixed(6)}`
            );
            console.log(
                `   Maximum observed: ${report.analysis.ratioStatistics.max.toFixed(6)}`
            );
        ...
        console.log(
            `   • ${report.currentProblems.passiveVolumeRatio.problem}`
        );
        console.log(
            `   • ${report.currentProblems.finalConfidenceRequired.problem}`
        );
        console.log(
            `   • ${report.currentProblems.priceEfficiencyThreshold.problem}`
        );

        console.log(`\n✅ RECOMMENDED CONFIGURATION:`);
        const rec = report.recommendations.recommended;
        console.log(
            `   passiveAbsorptionThreshold: ${rec.passiveAbsorptionThreshold.toFixed(4)}`
        );
        console.log(
            `   finalConfidenceRequired: ${rec.finalConfidenceRequired}`
        );
        console.log(
            `   priceEfficiencyThreshold: ${rec.priceEfficiencyThreshold}`
        );
        ...
        console.log(
            `   Expected Impact: ${report.implementation.phase1.expectedImpact}`
        );
