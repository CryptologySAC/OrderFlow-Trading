...
import type {
    ProcessedSignal,
    SignalStatistics,
} from "../src/trading/signalManager.js";
...
async function demonstrateEnhancedMetrics() {
    ...
    for (const signal of exampleSignals) {
        try {
            ...
            console.log(
                `✅ Processed signal ${signal.id} (${signal.type}, confidence: ${signal.confidence})`
            );
        } catch (error) {
    ...
    console.log(
        `   Processing Duration P50: ${stats.processing.processingDurationP50}ms`
    );
    console.log(
        `   Processing Duration P95: ${stats.processing.processingDurationP95}ms\n`
    );
    ...
    console.log(
        `     Low Confidence: ${stats.rejections.byReason.lowConfidence}`
    );
    console.log(
        `     Unhealthy Market: ${stats.rejections.byReason.unhealthyMarket}`
    );
    console.log(
        `     Processing Error: ${stats.rejections.byReason.processingError}`
    );
    ...
    console.log(
        `   Average Confidence: ${stats.quality.averageConfidence.toFixed(3)}`
    );
    ...
    console.log(
        `   Average Confidence Adjustment: ${stats.quality.averageConfidenceAdjustment.toFixed(3)}\n`
    );

    // Display correlation metrics
    console.log("🔗 Correlation Metrics:");
    console.log(
        `   Average Strength: ${stats.correlation.averageStrength.toFixed(3)}`
    );
    console.log(
        `   Correlated Signals P50: ${stats.correlation.correlatedSignalsP50}`
    );
    console.log(
        `   Correlated Signals P95: ${stats.correlation.correlatedSignalsP95}`
    );
    console.log(
        `   Confirmations with Correlation: ${stats.correlation.confirmationsWithCorrelation}\n`
    );

    // Display market health metrics
    console.log("💊 Market Health Metrics:");
    console.log(
        `   Confirmations (Healthy Market): ${stats.marketHealth.confirmationsHealthyMarket}`
    );
    console.log(
        `   Confirmations (Unhealthy Market): ${stats.marketHealth.confirmationsUnhealthyMarket}`
    );
    console.log(
        `   Blocked by Health: ${stats.marketHealth.blockedByHealth}\n`
    );

    // Display timing metrics
    console.log("⏱️  Timing Metrics:");
    console.log(
        `   Average Signal Age: ${stats.timing.averageSignalAge.toFixed(2)}ms`
    );
    console.log(
        `   Signal Age P95: ${stats.timing.signalAgeP95.toFixed(2)}ms\n`
    );
    ...
    console.log(
        `   Market Recommendation: ${status.marketHealth.recommendation}\n`
    );
