...
class SwingCorrelationAnalyzer:
    ...
    def load_price_data(self, hours=24):
        ...
        hours_ago_ms = current_ms - (hours * ms_per_hour)
        
        print(f"  ℹ️ Current time (ms): {current_ms}")
        print(f"  ℹ️ Looking for data after (ms): {hours_ago_ms}")
        print(f"  ℹ️ Time range: {datetime.fromtimestamp(hours_ago_ms/1000)} to {datetime.fromtimestamp(current_ms/1000)}")
        
        # First, check what data exists in the database
        cursor = conn.cursor()
        cursor.execute("SELECT MIN(tradeTime), MAX(tradeTime), COUNT(*) FROM aggregated_trades")
        min_time, max_time, total_count = cursor.fetchone()
        print(f"  ℹ️ Database range: {min_time} to {max_time} ({total_count} total rows)")
        if min_time and max_time:
            print(f"  ℹ️ Database dates: {datetime.fromtimestamp(min_time/1000)} to {datetime.fromtimestamp(max_time/1000)}")
        
        # Count rows in our time range
        cursor.execute("SELECT COUNT(*) FROM aggregated_trades WHERE tradeTime > ?", [hours_ago_ms])
        range_count = cursor.fetchone()[0]
        print(f"  ℹ️ Rows in last {hours} hours: {range_count}")
        ...
        try:
            ...
            print(f"  ℹ️ Raw data loaded: {len(self.price_data)} rows")
            
            if self.price_data.empty:
                print("  ⚠️ No data found in the specified time range!")
                # Try to get the most recent data regardless of time
                query_recent = """
                SELECT tradeTime as timestamp, price
                FROM aggregated_trades
                ...
                LIMIT 10000
                """
                self.price_data = pd.read_sql_query(query_recent, conn)
                if not self.price_data.empty:
                    print(f"  ℹ️ Loaded {len(self.price_data)} most recent rows instead")
                    recent_min = self.price_data['timestamp'].min()
                    recent_max = self.price_data['timestamp'].max()
                    print(f"  ℹ️ Recent data range: {datetime.fromtimestamp(recent_min/1000)} to {datetime.fromtimestamp(recent_max/1000)}")
            ...
            if not self.price_data.empty:
                ...
                if not self.price_data.empty:
                    ...
                    print(f"  ✓ Date range: {datetime.fromtimestamp(min_ts/1000)} to {datetime.fromtimestamp(max_ts/1000)}")
    ...
    def detect_swings(self, window=10):
        ...
        print(f"  ℹ️ Analyzing {len(prices)} price points")
        print(f"  ℹ️ Price stats: mean=${prices.mean():.2f}, std=${prices.std():.2f}, min=${prices.min():.2f}, max=${prices.max():.2f}")
        
        # Calculate price volatility
        price_changes = np.diff(prices) / prices[:-1]
        print(f"  ℹ️ Price volatility: max change={np.abs(price_changes).max()*100:.2f}%, avg change={np.abs(price_changes).mean()*100:.4f}%")
        ...
        print(f"  ℹ️ Using prominence threshold: ${min_prominence:.2f} (0.1% of mean price)")
        
        peaks, peak_properties = signal.find_peaks(prices, distance=window, prominence=min_prominence)
        valleys, valley_properties = signal.find_peaks(-prices, distance=window, prominence=min_prominence)
        
        print(f"  ℹ️ Found {len(peaks)} peaks and {len(valleys)} valleys")
        if len(peaks) > 0:
            print(f"  ℹ️ Peak prominences: {peak_properties['prominences'][:5]}... (showing first 5)")
        if len(valleys) > 0:
            print(f"  ℹ️ Valley prominences: {valley_properties['prominences'][:5]}... (showing first 5)")
        ...
        swing_movements = []
        all_pct_changes = []  # Track all percentage changes for debugging
        ...
        for i in range(1, len(swings)):
            ...
            if prev_swing['type'] != curr_swing['type']:
                ...
                all_pct_changes.append(pct_change * 100)  # Store as percentage
        ...
        # Debug: Show all swing sizes found
        if all_pct_changes:
            all_pct_changes.sort(reverse=True)
            print(f"  ℹ️ All swing sizes detected: {all_pct_changes[:10]}% (showing top 10)")
            print(f"  ℹ️ Swings >= {self.swing_threshold*100:.1f}%: {sum(1 for p in all_pct_changes if p >= self.swing_threshold*100)}")
            print(f"  ℹ️ Largest swing: {all_pct_changes[0]:.2f}%")
            print(f"  ℹ️ Smallest swing: {all_pct_changes[-1]:.4f}%")
    ...
    def load_all_signals(self):
        ...
        if len(all_dfs) > 0:
            ...
            if 'timestamp' in self.all_signals.columns:
                ...
                # Debug: Show timestamp range of signals
                if not self.all_signals.empty:
                    min_ts = self.all_signals['timestamp'].min()
                    max_ts = self.all_signals['timestamp'].max()
                    print(f"  ℹ️ Signal timestamp range: {min_ts} to {max_ts}")
                    print(f"  ℹ️ Signal date range: {datetime.fromtimestamp(min_ts/1000)} to {datetime.fromtimestamp(max_ts/1000)}")
                    
                    # Show breakdown by type and category
                    for signal_type in self.all_signals['signal_type'].unique():
                        type_count = len(self.all_signals[self.all_signals['signal_type'] == signal_type])
                        print(f"    • {signal_type}: {type_count} signals")
