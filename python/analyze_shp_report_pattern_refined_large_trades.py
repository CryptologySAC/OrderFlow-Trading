...
NUM_PROCESSES = 8
WINDOW_SECONDS = 30  # 30 seconds for both volume ratio and large trade check
...
def analyze_shp(shp_row, trades, quantity_threshold):
    ...
    # Extract trades within the 30-second window
    window_trades = trades[(trades['tradeTime'] >= start_time) & (trades['tradeTime'] < high_time)]
    if window_trades.empty:
        return None

    # Check for large trades (>99th percentile)
...
if __name__ == '__main__':
    ...
    print(f"Pattern (Volume Ratio > {VOLUME_RATIO_THRESHOLD} AND Has Large Trade): {pattern_presence:.2f}% of SHPs")
    print(f"Volume Ratio > {VOLUME_RATIO_THRESHOLD}: {volume_ratio_presence:.2f}% of SHPs")
    print(f"Has Large Trade (>99th Percentile): {large_trade_presence:.2f}% of SHPs")
    ...
    print(f"\nPattern Specificity Across Database (Volume Ratio > {VOLUME_RATIO_THRESHOLD} AND Has Large Trade, Window: 30 seconds):")
