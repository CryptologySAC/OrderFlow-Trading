# Version 1.7: Hybrid of Version 1.3 and 1.6 with Relaxed Filtering (20250429_210000)
# Changes from Version 1.6 (20250429_200000):
# - Updated VERSION to 20250429_210000
# - Removed confirmThreshold (set to 0) in some tests to mimic Version 1.3
# - Lowered volume filter to 25th percentile to increase signals
# - Updated grid search: confirmThresholds=[0, 0.003], trailingStopPcts=[0.005, 0.008], takeProfits=[0.02, 0.025]
# - Fixed fee sensitivity analysis for 0.075% fee
# Changes from Version 1.5.2:
# - Reduced minLargeOrderCount from 5 to 3
...
VERSION = "20250429_210000"  # Hybrid Version, April 29, 2025, 21:00:00
...
class OrderFlowAnalyzer:
    def __init__(self, absorptionWindow=5, priceRange=0.005, minLargeOrder=50, volumeWindow=15*60*1000, 
                 confirmThreshold=0.0, trailingStopPct=0.005, takeProfit=0.02):
        ...
        self.minLargeOrderCount = 3  # Reduced to increase signals
    ...
    def get_dynamic_trailing_stop(self):
        ...
        scale_factor = min(2.0, max(0.75, avg_quantity / self.minLargeOrder))  # Scaling: 0.75x–2x
    ...
    def get_large_order_volume_threshold(self):
        ...
        return np.quantile(volumes, 0.25) if volumes else 0  # Lowered to 25th percentile
                ...
                if all(priceMin <= t['price'] <= priceMax for t in opposingTrades):
                    ...
                    if self.isNearHighVolume(trade['price']):
                        if signal_type == 'sell_absorption':
                            if self.confirmThreshold == 0 or trade['max_price_5min'] >= signal['price'] * (1 + self.confirmThreshold):
                            ...
                            if self.confirmThreshold == 0 or trade['min_price_5min'] <= signal['price'] * (1 - self.confirmThreshold):
...
trailingStopPcts = [0.005, 0.008]
absorptionWindows = [5]
priceRanges = [0.005]
minLargeOrders = [50]
volumeWindows = [15*60*1000]
confirmThresholds = [0.0, 0.003]
takeProfits = [0.02, 0.025]
...
def evaluate_parameters(params, df=df):
    ...
    alt_transaction_fee = 0.00075  # 0.075% for sensitivity analysis
    
    for timeframe in ['Daytime', 'Nighttime']:
        tf_signals = signals[signals['timeframe'] == timeframe]
        if tf_signals.empty:
            results[timeframe] = (0, 0, 0, 0, 0, 0)
            continue
        ...
        returns_alt_fee = torch.zeros(len(tf_signals), dtype=torch.float32, device=device)
        ...
        if buy_mask.any():
            ...
            if manual_close.any():
                ...
                returns_alt_fee[manual_indices] = ((exit_buy[manual_close] / entry_buy[manual_close]) - 1) - 2 * alt_transaction_fee
            ...
            if len(tp_sl_indices) > 0:
                ...
                returns_alt_fee[tp_sl_indices[tp_hit]] = takeProfit - 2 * alt_transaction_fee
                ...
                returns_alt_fee[tp_sl_indices[sl_hit]] = -stop_loss_buy_tp_sl[sl_hit] - 2 * alt_transaction_fee
                ...
                returns_alt_fee[tp_sl_indices[orig_profit]] = ((max_buy_tp_sl[orig_profit] / entry_buy_tp_sl[orig_profit]) - 1) - 2 * alt_transaction_fee
        ...
        if sell_mask.any():
            ...
            if manual_close.any():
                ...
                returns_alt_fee[manual_indices] = ((entry_sell[manual_close] / exit_sell[manual_close]) - 1) - 2 * alt_transaction_fee
                outcomes[manual_indices[returns[manual_indices] > 0]] = 1
                outcomes[manual_indices[returns[manual_indices] <= 0]] = 2
            ...
            if len(tp_sl_indices) > 0:
                ...
                returns_alt_fee[tp_sl_indices[sl_hit]] = -stop_loss_sell_tp_sl[sl_hit] - 2 * alt_transaction_fee
                ...
                returns_alt_fee[tp_sl_indices[orig_profit]] = ((entry_sell_tp_sl[orig_profit] / min_sell_tp_sl[orig_profit]) - 1) - 2 * alt_transaction_fee
        
        profitable_pct = (outcomes == 1).float().mean().item()
        num_signals = len(tf_signals)
        avg_return = returns.mean().item() if len(returns) > 0 else 0
        avg_return_alt_fee = returns_alt_fee.mean().item() if len(returns_alt_fee) > 0 else 0
        hit_rate = profitable_pct
        loss_rate = (outcomes == 2).float().mean().item()
        
        results[timeframe] = (profitable_pct, num_signals, avg_return, hit_rate, loss_rate, avg_return_alt_fee)
...
if __name__ == '__main__':
    ...
    for i, params in enumerate(param_combinations):
        ...
        for timeframe, (profitable_pct, num_signals, avg_return, hit_rate, loss_rate, avg_return_alt_fee) in [('Daytime', daytime_results), ('Nighttime', nighttime_results)]:
            results_data.append({
                'timeframe': timeframe,
            ...
            print(f"Fee Sensitivity (0.075% fee): Avg Return: {avg_return_alt_fee:.4f}")
    ...
    for timeframe in ['Daytime', 'Nighttime']:
        ...
        if not tf_df.empty:
            ...
            print(f"Fee Sensitivity (0.075% fee): Avg Return: {best_result['avg_return_alt_fee']:.4f}")
    ...
    sma_signals = []
    stop_loss = 0.003
    take_profit = 0.02  # Aligned with grid search
    transaction_fee = 0.001  # 0.1% per trade (entry + exit)
    active_trade = None
