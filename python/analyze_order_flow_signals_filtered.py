...
# Configuration
DB_PATH = "../trades.db"
CSV_PATH = "swing_high_low_pairs_filtered_v1.1.csv"  # Filtered HP data (excluded top 5% drops)
...
def main():
    """Analyze order flow signals before filtered HPs to identify predictive patterns."""
    try:
        conn = sqlite3.connect(DB_PATH)
        print("Connected to database successfully.")

        # Load filtered pairs (HPs with top 5% drops excluded)
        pairs_df = load_pairs(CSV_PATH)
        hps = pairs_df[['high_time', 'high_price']].copy()
        print(f"Analyzing {len(hps)} HPs from {CSV_PATH} (filtered to exclude top 5% drops).")
