...
def check_shp_flow(window_end, trades, min_volume=50, min_net_volume=37.5, 
    ...
    Returns:
    - dict: Signal details including patterns, flow window, and flow metrics for filtering.
    """
...
def filter_signals(signals):
    ...
    # Extract metrics for all signals
    imbalance_ratios = np.array([s['ImbalanceRatio'] for s in signals])
    cumulative_pressures = np.array([s['CumulativePressure'] for s in signals])
    reversal_volumes = np.array([abs(s['ReversalVolume']) for s in signals])
    trade_intensities = np.array([s['TradeIntensity'] for s in signals])

    # Handle infinite imbalance ratios by replacing with the maximum finite value
    finite_imbalance_ratios = imbalance_ratios[np.isfinite(imbalance_ratios)]
    max_finite_imbalance = np.max(finite_imbalance_ratios) if len(finite_imbalance_ratios) > 0 else 0
    imbalance_ratios = np.where(np.isinf(imbalance_ratios), max_finite_imbalance, imbalance_ratios)
    ...
    def normalize_metric(metric):
        ...
        if max_val == min_val:
            return np.zeros_like(metric) if min_val == 0 else np.ones_like(metric)
        return (metric - min_val) / (max_val - min_val)

    norm_imbalance = normalize_metric(imbalance_ratios)
    norm_pressure = normalize_metric(cumulative_pressures)
    norm_reversal_volume = normalize_metric(reversal_volumes)
    norm_trade_intensity = normalize_metric(trade_intensities)

    # Compute composite score with weights
    scores = (0.3 * norm_imbalance +
              0.3 * norm_pressure +
              0.2 * norm_reversal_volume +
              0.2 * norm_trade_intensity)

    # Determine the threshold for the top 20% of scores
    score_threshold = np.percentile(scores, 80) if len(scores) > 0 else 0
...
if __name__ == '__main__':
    ...
    if not trades_df.empty:
        ...
        trades_df.to_csv('trade_execution_results_live_filtered_v3.csv', index=False)
        print("\nTrade execution results saved to 'trade_execution_results_live_filtered_v3.csv'.")