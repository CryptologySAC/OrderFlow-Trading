# Version 1.3.1: Version 1.3 with 0.1% Fee (20250429_213000)
# Changes from Version 1.3 (20250429_160000):
# - Updated VERSION to 20250429_213000
# - Set transaction_fee to 0.001 (0.1% per trade, 0.2% round-trip)
# - Added avg_return_alt_fee for 0.075% fee sensitivity
# - Explicitly set confirmThreshold=0.0, minLargeOrderCount=0 (no volume filter)
# - Used fixed trailingStopPct=0.005, takeProfit=0.015
...
VERSION = "20250429_213000"  # Version 1.3 with 0.1% Fee, April 29, 2025, 21:30:00

# Define Lima timezone (PET, UTC-5)
lima_tz = pytz.timezone('America/Lima')

# Define the OrderFlowAnalyzer class with fixed stop-loss
class OrderFlowAnalyzer:
    def __init__(self, absorptionWindow=5, priceRange=0.005, minLargeOrder=50, volumeWindow=15*60*1000, 
                 confirmThreshold=0.0, trailingStopPct=0.005, takeProfit=0.015):
        ...
        self.minLargeOrderCount = 0  # No volume filter
    ...
    def apply_trailing_stop(self, current_price, position_type):
        if not self.activeTrade:
            return False
        stop_pct = max(self.trailingStopPct, self.minStopDistance)
        if position_type == 'sell_absorption':
            ...
            stop_loss = self.bestPrice * (1 - stop_pct)
...
# Define parameter ranges (minimal grid search for Version 1.3)
trailingStopPcts = [0.005]
absorptionWindows = [5]
priceRanges = [0.005]
minLargeOrders = [50]
volumeWindows = [15*60*1000]
confirmThresholds = [0.0]
takeProfits = [0.015]
...
if __name__ == '__main__':
    ...
    sma_signals = []
    stop_loss = 0.003
    take_profit = 0.015  # Aligned with grid search
    transaction_fee = 0.001  # 0.1% per trade (entry + exit)
    active_trade = None
    ...
    if not sma_signals.empty:
        for timeframe in ['Daytime', 'Nighttime']:
            ...
            if sell_mask.any():
                ...
                manual_close = np.array([r in ['opposite_signal', 'end_of_data'] for r in close_reasons])
                if manual_close.any():
                    manual_indices = sell_indices[torch.tensor(manual_close, device=device)]
