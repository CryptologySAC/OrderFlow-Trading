...
# Compute static 5th percentile of average SELL LT quantities across all SHPs
def compute_static_sell_quantity_threshold(shp_df, trades, quantity_threshold):
    ...
    if avg_sell_quantities:
        return np.percentile(avg_sell_quantities, 5)
...
def analyze_shp(shp_row, trades, sell_quantity_threshold):
    ...
    matches_pattern = sell_lt_quantity > sell_quantity_threshold
...
if __name__ == '__main__':
    ...
    # Compute the static 5th percentile SELL LT quantity threshold
    sell_quantity_threshold = compute_static_sell_quantity_threshold(shp_df, trades, quantity_threshold)
    print(f"Static 5th Percentile SELL LT Quantity Threshold: {sell_quantity_threshold}")
    ...
    # Save to CSV
    results_df.to_csv('shp_refined_pattern_analysis_rolling_parallel_final_v3.csv', index=False)
    print("Analysis saved to 'shp_refined_pattern_analysis_rolling_parallel_final_v3.csv'.")
