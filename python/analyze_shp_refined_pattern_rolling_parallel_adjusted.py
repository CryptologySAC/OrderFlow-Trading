...
VOLUME_RATIO_THRESHOLD = 1.0  # Adjusted to 1.0
TIME_SINCE_BUY_THRESHOLD = 64.57  # Adjusted to mean latest BUY LT timing
CUMULATIVE_DELTA_THRESHOLD = -10000  # Adjusted to a static value
NUM_PROCESSES = 8
...
def compute_rolling_sell_quantities(shp_df, trades, rolling_end_time, quantity_threshold):
    ...
    if avg_sell_quantities:
        return np.percentile(avg_sell_quantities, 50)
...
def analyze_shp(shp_row, trades, shp_df):
    ...
    # Compute the 50th percentile SELL LT quantity threshold in the rolling window ending at high_time
...
if __name__ == '__main__':
    ...
    # Save to CSV
    results_df.to_csv('shp_refined_pattern_analysis_rolling_parallel_adjusted.csv', index=False)
    print("Analysis saved to 'shp_refined_pattern_analysis_rolling_parallel_adjusted.csv'.")
    ...
    print(f"Mean 50th Percentile SELL LT Quantity Threshold: {results_df['sell_quantity_threshold'].mean():.2f}")