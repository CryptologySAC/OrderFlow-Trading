...
# Define the OrderFlowAnalyzer class with pressure-based exits
class OrderFlowAnalyzer:
    ...
    def checkPressureExit(self, trade_timestamp, entry_price, is_buy_trade):
        # Check buy/sell volume in 5 seconds for pressure-based exit
        buy_volume = 0
        sell_volume = 0
        min_price = entry_price
        max_price = entry_price
        exit_window = pd.Timedelta(seconds=5)
        for trade in self.recentTrades:
            if trade_timestamp - exit_window <= trade['timestamp'] <= trade_timestamp:
                if trade['isBuyer']:
                    buy_volume += trade['quantity']
                else:
                    sell_volume += trade['quantity']
                ...
                max_price = max(max_price, trade['price'])
        if is_buy_trade:
            volume_condition = sell_volume > 3 * buy_volume and sell_volume > 0
            price_condition = min_price <= entry_price * (1 - 0.002)
            return volume_condition or price_condition, min_price
        else:
            volume_condition = buy_volume > 3 * sell_volume and buy_volume > 0
            price_condition = max_price >= entry_price * (1 + 0.002)
            return volume_condition or price_condition, max_price
    ...
    def processTrade(self, trade):
        # Check for pressure-based exit if there's an active trade
        if self.activeTrade:
            is_buy_trade = self.activeTrade['type'] == 'sell_absorption'  # Buy trade
            should_exit, exit_price = self.checkPressureExit(trade['timestamp'], self.activeTrade['price'], is_buy_trade)
            if should_exit:
                self.closeActiveTrade(exit_price, trade['timestamp'], trade.name, reason='pressure_exit')
                return
        ...
        if trade['quantity'] >= self.minLargeOrder:
            ...
            if opposingTrades:
                ...
                if all(priceMin <= t['price'] <= priceMax for t in opposingTrades):
                    ...
                    is_buy_signal = signal_type == 'sell_absorption'  # Buy signal
                    # Check if signal should be processed
                    if self.activeTrade:
                        # Opposite signal: close active trade
                        if (self.activeTrade['type'] == 'sell_absorption' and signal_type == 'buy_absorption') or \
                           (self.activeTrade['type'] == 'buy_absorption' and signal_type == 'sell_absorption'):
                            self.closeActiveTrade(trade['price'], trade['timestamp'], trade.name, reason='opposite_signal')
                        # Same signal: ignore
                        else:
                            return
...
def evaluate_parameters(params, df=df):
    ...
    if buy_mask.any():
        ...
        # Trades closed by pressure exit, opposite signal, or end of data
        manual_close = np.array([r in ['pressure_exit', 'opposite_signal', 'end_of_data'] for r in close_reason_buy])
        if manual_close.any():
    ...
    if sell_mask.any():
        ...
        # Trades closed by pressure exit, opposite signal, or end of data
        manual_close = np.array([r in ['pressure_exit', 'opposite_signal', 'end_of_data'] for r in close_reason_sell])
        if manual_close.any():
...
if __name__ == '__main__':
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting grid search with {len(param_combinations)} combinations")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Results saved to results_{timestamp}.csv")
