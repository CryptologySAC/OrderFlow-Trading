# Version 1.8: Refined Hybrid with Wider Stops and Higher Take Profit (20250429_220000)
# Changes from Version 1.7 (20250429_210000):
# - Updated VERSION to 20250429_220000
# - Reverted confirmThreshold to [0.003, 0.004] for better trade quality
# - Set volume filter to 33rd percentile to balance signals
# - Updated grid search: confirmThresholds=[0.003, 0.004], trailingStopPcts=[0.007, 0.01], takeProfits=[0.025, 0.03]
# - Adjusted dynamic stop scaling to 1x–2.5x
# - Increased minStopDistance to 0.002
# Changes from Version 1.6:
# - Reduced minLargeOrderCount from 5 to 3
# - Fixed fee sensitivity analysis
...
VERSION = "20250429_220000"  # Refined Hybrid, April 29, 2025, 22:00:00
...
class OrderFlowAnalyzer:
    def __init__(self, absorptionWindow=5, priceRange=0.005, minLargeOrder=50, volumeWindow=15*60*1000, 
                 confirmThreshold=0.003, trailingStopPct=0.007, takeProfit=0.025):
        ...
        self.minStopDistance = 0.002  # Increased to 0.2%
        self.minLargeOrderCount = 3
    ...
    def get_dynamic_trailing_stop(self):
        ...
        scale_factor = min(2.5, max(1.0, avg_quantity / self.minLargeOrder))  # Scaling: 1x–2.5x
    ...
    def get_large_order_volume_threshold(self):
        ...
        return np.quantile(volumes, 0.33) if volumes else 0  # Set to 33rd percentile
...
def evaluate_parameters(params, df=df):
    ...
    for timeframe in ['Daytime', 'Nighttime']:
        ...
        stop_loss = torch.where(is_invalidated, torch.tensor(0.002, device=device), torch.tensor(trailingStopPct, device=device))
...
if __name__ == '__main__':
    ...
    sma_signals = []
    stop_loss = 0.003
    take_profit = 0.025  # Aligned with grid search
    transaction_fee = 0.001  # 0.1% per trade (entry + exit)
    active_trade = None
