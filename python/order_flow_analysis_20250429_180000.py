# Version 1.4: Trailing Stop-Loss Optimization and Transaction Fees (20250429_180000)
# Changes from Version 1.3 (20250429_173000):
# - Updated VERSION to 20250429_180000
# - Set default parameters to best daytime configuration (absorptionWindow=5, priceRange=0.005, minLargeOrder=50, volumeWindow=15.0, confirmThreshold=0.003, takeProfit=0.015)
# - Added trailing stop percentage grid search ([0.002, 0.004, 0.005])
# - Incorporated transaction fees (0.1% per trade) in returns calculation
# - Added order flow-based volume filter to avoid low-quality signals
# - Simplified grid search to focus on trailingStopPct
...
VERSION = "20250429_180000"  # Trailing Stop-Loss Optimization, April 29, 2025, 18:00:00
...
class OrderFlowAnalyzer:
    def __init__(self, absorptionWindow=5, priceRange=0.005, minLargeOrder=50, volumeWindow=15*60*1000, 
        ...
        self.confirmThreshold = confirmThreshold   # price move for confirmation
        self.trailingStopPct = trailingStopPct    # trailing stop percentage
        self.takeProfit = takeProfit              # take-profit percentage
        ...
        self.bestPrice = None                     # track best price for trailing stop
        self.largeOrderVolumes = deque()          # track large order volumes for filtering
    ...
    def updateRecentTrades(self, trade):
        ...
        # Update large order volumes for filtering
        ...
        if trade['quantity'] >= self.minLargeOrder:
            self.largeOrderVolumes.append({'timestamp': current_time, 'volume': trade['quantity']})
    ...
    def apply_trailing_stop(self, current_price, position_type):
        """Apply trailing stop-loss to the active trade."""
        if not self.activeTrade:
            return False
        if position_type == 'sell_absorption':  # Buy signal (long)
            if self.bestPrice is None:
                self.bestPrice = current_price
            self.bestPrice = max(self.bestPrice, current_price)
            stop_loss = self.bestPrice * (1 - self.trailingStopPct)
    ...
    def get_large_order_volume_threshold(self):
        """Calculate the 75th percentile of large order volumes."""
        volumes = [v['volume'] for v in self.largeOrderVolumes]
        return np.quantile(volumes, 0.75) if volumes else 0
    ...
    def processTrade(self, trade):
        ...
        # Check active trade for trailing stop or take-profit
        if self.activeTrade:
        ...
        # Order flow volume filter
        large_order_volume = sum(v['volume'] for v in self.largeOrderVolumes)
        if large_order_volume < self.get_large_order_volume_threshold():
            return
...
trailingStopPcts = [0.002, 0.004, 0.005]
absorptionWindows = [5]
priceRanges = [0.005]
minLargeOrders = [50]
volumeWindows = [15*60*1000]
confirmThresholds = [0.003]
takeProfits = [0.015]
...
def evaluate_parameters(params, df=df):
    ...
    for timeframe in ['Daytime', 'Nighttime']:
        ...
        # Apply dynamic stop-loss for invalidated signals
        ...
        if buy_mask.any():
            ...
            # Trades closed by trailing stop, take-profit, opposite signal, or end of data
...
if __name__ == '__main__':
    ...
    sma_signals = []
    stop_loss = 0.003  # Match trailing stop default
    take_profit = 0.015  # Match take-profit default
    active_trade = None
