...
export class SwingPoints {
    ...
    public addPriceLevel(
        ...
        // maybe we have a new bandwith limit
            ...
            if (
                this.lastSwingPoint === HighLow.HIGH &&
                price <= this.highSwing.price * (1 - this.bandWidth)
            ) {
                ...
                console.log(
                    "New low limit detected (BandWidth): Swing High order confirmed: %s of %s at %s",
                    this.highSwing.tradeId,
                    this.highSwing.price,
                    new Date(this.highSwing.timeStamp)
                );
            } else if (
                this.lastSwingPoint === HighLow.LOW &&
                price >= this.lowSwing.price * (1 + this.bandWidth)
            ) {
                ...
                console.log(
                    "New high limit detected (BandWidth): Swing Low order confirmed: %s of %s at %s",
                    this.lowSwing.tradeId,
                    this.lowSwing.price,
                    new Date(this.lowSwing.timeStamp)
                );
            }
        }
    }
