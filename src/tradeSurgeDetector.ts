...
export class TradeSurgeDetector {
    private consolidationRange: {
        start: number;
        end: number;
        buyVolume: number;
        sellVolume: number;
        totalVolume: number;
        trades: { price: number; quantity: number; isBuy: boolean }[];
    } = {
        start: 0,
        end: 0,
        buyVolume: 0,
    ...
    private outsideRangeVolume: { above: number; below: number } = {
        above: 0,
        below: 0,
    };
    private burstWindow: {
    ...
    private surgePriceRange: { start: number; end: number } = {
        start: 0,
        end: 0,
    };
    ...
    private calculateAdaptiveParameters(avgMarketDepth: number): {
        burstVolumeWindow: number;
        absorptionVolume: number;
        deltaThreshold: number;
        sharpDropThreshold: number;
        breakoutPriceThreshold: number;
        ...
        const totalVolume = this.recentVolumes.reduce(
            (sum, v) => sum + v.volume,
            0
        );

        // Calculate price volatility over the last 15 minutes
        this.recentPrices = this.recentPrices.filter(
            (p) => p.timestamp >= cutoffTime
        );
        const priceHigh = Math.max(
            ...this.recentPrices.map((p) => p.price),
            this.lastTradePrice
        );
        ...
        // Adaptive parameters
        const baseBurstVolumeWindow =
            totalVolume * 0.05 * volumeAdjustmentFactor;
        const burstVolumeWindow =
            Math.round(baseBurstVolumeWindow / 50) * 50 || 50;
        const baseAbsorptionVolume =
            totalVolume * 0.025 * volumeAdjustmentFactor;
        const absorptionVolume =
            Math.round(baseAbsorptionVolume / 25) * 25 || 25;
        const deltaThreshold = Math.round(avgMarketDepth * 0.2) || 500;
        const sharpDropThreshold = this.lastTradePrice * 0.0025;
        const breakoutPriceThreshold = this.lastTradePrice * 0.005;
        const baseBreakoutVolumeThreshold =
            totalVolume * 0.1 * volumeAdjustmentFactor;
        const breakoutVolumeThreshold =
            Math.round(baseBreakoutVolumeThreshold / 100) * 100 || 100;
    ...
    private processTradeUpdate(
        ...
        this.consolidationRange.start = Math.min(
            this.consolidationRange.start,
            Math.floor(price * 10) / 10
        );
        this.consolidationRange.end = Math.max(
            this.consolidationRange.end,
        ...
        if (
            (price >=
                this.consolidationRange.end + params.breakoutPriceThreshold &&
                this.outsideRangeVolume.above >=
                    params.breakoutVolumeThreshold) ||
            (price <
                this.consolidationRange.start - params.breakoutPriceThreshold &&
                this.outsideRangeVolume.below >= params.breakoutVolumeThreshold)
        ) {
            const binStart = Math.floor(price * 10) / 10;
            const binEnd = binStart + this.binSize;
        ...
        while (
            this.burstWindow.totalVolume > params.burstVolumeWindow &&
            this.burstWindow.trades.length > 0
        ) {
            ...
            this.burstWindow.high = Math.max(
                ...this.burstWindow.trades.map((t) => t.price),
                0
            );
        }
        ...
        if (
            this.burstWindow.delta >= params.deltaThreshold &&
            !this.surgeDetected
        ) {
            this.buySellRatio =
                this.burstWindow.sellVolume > 0
                    ? this.burstWindow.buyVolume / this.burstWindow.sellVolume
                    : this.burstWindow.buyVolume > 0
                      ? Infinity
                      : 0;
            this.surgeDetected = this.buySellRatio > 3.5;
            this.surgePriceRange = {
                start: this.consolidationRange.start,
                end: this.consolidationRange.end,
            };
        ...
        if (this.surgeDetected && !this.isAbsorbed) {
            ...
            while (index >= 0 && postSurgeVolume < params.absorptionVolume) {
                ...
                if (index > 0) {
                    ...
                    if (
                        priceDrop >= params.sharpDropThreshold &&
                        postSurgeSellVolume > postSurgeBuyVolume
                    ) {
