...
try { 
    let allAggregatedTrades: SpotWebsocketAPI.TradesAggregateResponseResultInner[] = storage.getLatestAggregatedTrades(15000000);
    ...
    for (const trade of allAggregatedTrades) {
        let aggregateTrade: SwingPoint = {
            tradeId: trade.a !== undefined ? trade.a : 0,
            price: trade.p !== undefined ? parseFloat(trade.p) : 0,
            timeStamp: trade.T !== undefined ? trade.T : 0,
        }
        swingPoints.addPriceLevel(aggregateTrade.price, aggregateTrade.tradeId, aggregateTrade.timeStamp);
    }
