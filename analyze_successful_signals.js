...
function convertToLimaTime(timestamp) {
    // Lima is UTC-5
...
async function analyzeSignals() {
    const db = new better_sqlite3_1.default("trades.db", { readonly: true });
    const results = [];
    try {
        ...
        for (const filePath of signalFiles) {
            try {
                ...
                for (const signal of signals) {
                    ...
                    `);
                    const prices = priceQuery.all(signal.timestamp, endTime);
                    if (prices.length === 0) {
                        console.log(
...
function generateDetectorTablesHTML(results) {
    const detectors = ["absorption", "exhaustion", "deltacvd"];
    let tablesHTML = "";
    for (const detector of detectors) {
        const detectorResults = results.filter(
